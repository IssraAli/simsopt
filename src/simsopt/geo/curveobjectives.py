...
class CurveCurveDistance(Optimizable):
    ...
    def recompute_bell(self, parent=None):
        """
        Clear proximity ``candidates`` only when a curve's DOF data version
        changed. Pure-current (or other) updates that do not bump any curve
        ``Dofs._state_version`` can skip re-running the expensive sopp
        pre-filter on identical ``gamma()`` samples.
        """
        current = {id(c._dofs): c._dofs._state_version for c in self.curves}
        last = getattr(self, "_ccd_last_curve_dof_versions", None)
        if last is not None and last == current:
            return
        self._ccd_last_curve_dof_versions = {k: v for k, v in current.items()}
        self.candidates = None
...
class CurveSurfaceDistance(Optimizable):
    ...
    def recompute_bell(self, parent=None):
        current = {id(c._dofs): c._dofs._state_version for c in self.curves}
        current[id(self.surface._dofs)] = self.surface._dofs._state_version
        last = getattr(self, "_csd_last_dof_versions", None)
        if last is not None and last == current:
            return
        self._csd_last_dof_versions = {k: v for k, v in current.items()}
        self.candidates = None
...
class LinkingNumber(Optimizable):
    ...
    def J(self):
        """
        The Gauss linking number is piecewise constant in coil space; cache
        it while the underlying curve ``Dofs._state_version`` tuple is
        unchanged to avoid repeated ``simsoptpp.compute_linking_number`` work
        during optimization.
        """
        state_t = tuple((id(c._dofs), c._dofs._state_version) for c in self.curves)
        cached = getattr(self, "_linking_number_j_cache", None)
        if cached is not None and cached[0] == state_t:
            return cached[1]
        val = sopp.compute_linking_number(
            [c.gamma() for c in self.curves],
            [c.gammadash() for c in self.curves],
            self.dphis,
            self.downsample,
        )
        self._linking_number_j_cache = (state_t, val)
        return val
