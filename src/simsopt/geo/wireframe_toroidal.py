...
class ToroidalWireframe(object):
    ...
    def __init__(
        self, surface, n_phi, n_theta, constraint_atol=1e-10, constraint_rtol=1e-10
    ):
        ...
        if not surface.stellsym:
            raise ValueError(
                "Surfaces without stellarator symmetry are not "
        ...
        if (not np.isscalar(constraint_atol) and not constraint_atol > 0) or (
            not np.isscalar(constraint_rtol) and not constraint_rtol > 0
        ):
        ...
        qpoints_theta = list(np.linspace(0, 1.0, n_theta, endpoint=False))
        self.nfp = surface.nfp
        self.surface = SurfaceRZFourier(
            nfp=surface.nfp,
            stellsym=True,
            mpol=surface.mpol,
            ntor=surface.ntor,
        ...
        segments_tor[:, 0] = self.node_inds[:-1, :].reshape((self.n_tor_segments))
        segments_tor[:, 1] = self.node_inds[1:, :].reshape((self.n_tor_segments))
        ...
        self.tor_segment_key[:-1, :] = np.arange(self.n_tor_segments).reshape(
            (n_phi, n_theta)
        )
        ...
        self.pol_segment_key[0, :HalfNTheta] = (
            np.arange(HalfNTheta) + self.n_tor_segments
        )
        for i in range(1, n_phi):
            ...
            self.pol_segment_key[i, :] = (
                np.arange(polInd0, polInd1) + self.n_tor_segments
            )
        ...
        self.segments = np.ascontiguousarray(
            np.zeros((self.n_segments, 2)).astype(np.int64)
        )
    ...
    def determine_connected_segments(self):
        ...
        self.connected_segments = np.ascontiguousarray(
            np.zeros((self.n_nodes, 4)).astype(np.int64)
        )
        ...
        for i in range(self.n_phi + 1):
            for j in range(self.n_theta):
                ...
                if i == 0:
                    ind_tor_in = self.tor_segment_key[
                        i, (self.n_theta - j) % self.n_theta
                    ]
                ...
                # Second symmetry plane
                    ...
                    ind_tor_in = self.tor_segment_key[i - 1, j]
                    ind_tor_out = self.tor_segment_key[
                        i - 1, (self.n_theta - j) % self.n_theta
                    ]
                    if j == 0:
            ...
            for j in range(self.n_theta):
                ...
                # Second symmetry plane
                    ...
                    if j < half_n_theta:
                        ...
                        ind_pol2 = self.pol_segment_key[i + 1, self.n_theta - j - 1]
                    ind_tor3 = self.tor_segment_key[i, (j + 1) % self.n_theta]
                    ind_pol4 = self.pol_segment_key[i, j]
    ...
    def add_constraint(self, name, constraint_type, matrix_row, constant):
        ...
        if matrix_row.size != self.n_segments:
            raise ValueError(
                "matrix_row must have one element for every "
    ...
    def add_poloidal_current_constraint(self, current):
        ...
        self.add_constraint(
            "poloidal_current", "poloidal_current", matrix_row, pol_current_sum
        )
    ...
    def add_toroidal_current_constraint(self, current):
        ...
        self.add_constraint("toroidal_current", "toroidal_current", matrix_row, current)
    ...
    def add_segment_constraints(self, segments, implicit=False):
        ...
        if np.any(segments < 0) or np.any(segments >= self.n_segments):
            raise ValueError(
                "Segment indices must be positive and less than "
        ...
        for i in range(len(segments)):
            ...
            self.add_constraint(name + "_%d" % (segments[i]), name, matrix_row, 0)
    ...
    def constrain_colliding_segments(self, coll_func, pts_per_seg=10, **kwargs):
        ...
        coll = coll_func(
            test_pts[:, :, 0], test_pts[:, :, 1], test_pts[:, :, 2], **kwargs
        )
    ...
    def free_all_segments(self):
        ...
        for constr in list(self.constraints.keys()):
            if (
                self.constraints[constr]["type"] == "segment"
            ...
            if len(implicits) > 0:
                for node_ind in implicits:
                    for seg_ind in self.connected_segments[node_ind, :]:
                        if ("segment_%d" % (seg_ind) not in self.constraints) and (
                            "implicit_segment_%d" % (seg_ind) not in self.constraints
                        ):
    ...
    def constrained_segments(self, include="all", update=True):
        ...
        if include == "explicit":
            ...
            raise ValueError("Include must be 'all', 'explicit', " + "or 'implicit'")
    ...
    def add_continuity_constraints(self):
        ...
        for i in range(self.n_phi + 1):
            for j in range(self.n_theta):
                ...
                ind_tor_in, ind_pol_in, ind_tor_out, ind_pol_out = list(
                    self.connected_segments[self.node_inds[i, j]]
                )

                self.add_continuity_constraint(
                    self.node_inds[i, j],
                    ind_tor_in,
                    ind_pol_in,
                    ind_tor_out,
    ...
    def add_continuity_constraint(
        self, node_ind, ind_tor_in, ind_pol_in, ind_tor_out, ind_pol_out
    ):
    ...
    def constraint_matrices(
        ...
        constraints_C = np.ascontiguousarray(
            np.concatenate(
                [
                    self.constraints[key]["matrix_row"]
                    for key in self.constraints
        ...
        constraints_d = np.ascontiguousarray(np.zeros((constraints_C.shape[0], 1)))
    ...
    def find_inactive_nodes(self, assume_no_crossings=False):
        ...
        if assume_no_crossings:
            ...
            for i in range(self.n_segments):
                if (
                    "segment_%d" % (i) not in self.constraints
            ...
            while len(free_segs) > 0:
                ...
                while True:
                    ...
                    if not constraint_lifted:
                        if "continuity_node_%d" % nodes_i[0] in self.constraints:
    ...
    def get_free_cells(self, form="logical"):
        ...
        if form == "indices":
            ...
            raise ValueError("form parameter must be indices " + "or logical")
    ...
    def check_constraints(self, currents=None, atol=None, rtol=None):
        ...
        if currents is not None:
            if len(currents) != self.n_segments:
                raise ValueError(
                    "currents array does not match number of " + "segments"
                )
        ...
        constraints_C_full, constraints_d_full = self.constraint_matrices(
    ...
    def add_tfcoil_currents(
        self, n_tf, current_per_coil, constraint_atol=None, constraint_rtol=None
    ):
        """
        ...
        tf_inds = np.ceil(
            np.linspace(0, self.n_phi, n_tf, endpoint=False) + 0.5 * self.n_phi / n_tf
        )
        ...
        for i in range(len(tf_inds)):
            if tf_inds[i] == 0:
                ...
                ind0 = int(
                    (self.n_phi + 0.5) * self.n_theta + (tf_inds[i] - 1) * self.n_theta
                )
        ...
        valid = self.check_constraints(
            currents=new_currents, atol=constraint_atol, rtol=constraint_rtol
        )
    ...
    @SimsoptRequires(linesToVTK is not None, "to_vtk method requires pyevtk module")
    def to_vtk(
        self, filename, extent="torus", extra_node_data=None, extra_segment_data=None
    ):
        """
        ...
        if extent == "half period":
            ...
            raise ValueError(
                "extent must be 'half period', " + "'field period', or 'torus'"
            )
        ...
        for i in range(n_half_periods):
            ...
            pl_constr_segs_exp[ind0:ind1][
                self.constrained_segments(include="explicit")
            ] = 1
            pl_constr_segs_imp[ind0:ind1][
                self.constrained_segments(include="implicit")
            ] = 1
        ...
        linesToVTK(filename, x, y, z, cellData=segment_data, pointData=extra_node_data)
    ...
    def make_plot_3d(
        ...
        **kwargs,
        ...
        if extent == "half period":
            ...
            raise ValueError(
                "extent must be 'half period', " + "'field period', or 'full torus'"
            )
        ...
        if engine == "mayavi":
            ...
            if colormap is None:
                ...
                f = 0.9
                cmap = 255 * np.concatenate(
                    (f * r_cmap, f * g_cmap, f * b_cmap, alpha), axis=1
                )
            else:
                cmap = colormap
    ...
    def make_plot_2d(
        ...
        **kwargs,
        ...
        for i in range(n_half_periods):
            ...
            ind1 = (i + 1) * self.n_segments
            if i % 2 == 0:
                pl_segments[ind0:ind1, 0, 0] = i * self.n_phi + np.floor(
                    self.segments[:, 0] / self.n_theta
                )
                pl_segments[ind0:ind1, 0, 1] = self.segments[:, 0] % self.n_theta
                pl_segments[ind0:ind1, 1, 0] = i * self.n_phi + np.floor(
                    self.segments[:, 1] / self.n_theta
                )
                pl_segments[ind0:ind1, 1, 1] = self.segments[:, 1] % self.n_theta

                loop_segs = np.where(
                    np.logical_and(
                        pl_segments[ind0:ind1, 0, 1] == self.n_theta - 1,
                ...
                pl_segments[ind0:ind1, 0, 0] = (i + 1) * self.n_phi - np.floor(
                    self.segments[:, 0] / self.n_theta
                )
                pl_segments[ind0:ind1, 0, 1] = self.n_theta - (
                    self.segments[:, 0] % self.n_theta
                )
                pl_segments[ind0:ind1, 1, 0] = (i + 1) * self.n_phi - np.floor(
                    self.segments[:, 1] / self.n_theta
                )
        ...
        if coordinates == "indices":
            ...
            theta_mod = qp_theta[(pl_segments[:, :, 1] % self.n_theta).astype(int)]
        ...
        if add_colorbar:
            ...
            if quantity == "currents" or quantity == "nonzero currents":
                ...
                cb.set_label(
                    "1 = constrained; -1 = implicitly constrained; " + "0 = free"
                )
        else:
            cb = None
    ...
    def plot_cells_2d(self, cell_values, value_label=None, ax=None):
        ...
        vertices[:, 0, 0] = np.floor(
            self.segments[self.cell_key[:, 0], 0] / self.n_theta
        )
        vertices[:, 1, 0] = np.floor(
            self.segments[self.cell_key[:, 0], 1] / self.n_theta
        )
        vertices[:, 2, 0] = np.floor(
            self.segments[self.cell_key[:, 2], 1] / self.n_theta
        )
        vertices[:, 3, 0] = np.floor(
            self.segments[self.cell_key[:, 2], 0] / self.n_theta
        )
        vertices[:, 0, 1] = self.segments[self.cell_key[:, 0], 0] % self.n_theta
        vertices[:, 1, 1] = self.segments[self.cell_key[:, 0], 1] % self.n_theta
        vertices[:, 2, 1] = self.segments[self.cell_key[:, 2], 1] % self.n_theta
        vertices[:, 3, 1] = self.segments[self.cell_key[:, 2], 0] % self.n_theta

        loop_segs = np.where(
            np.logical_and(
                vertices[:, 0, 1] == self.n_theta - 1, vertices[:, 2, 1] == 0
            )
        )
        ...
        if len(cell_values) == self.cell_key.shape[0]:
            ...
            raise ValueError(
                "Input cell_values doesnt have the correct number of elements"
            )
...
def windowpane_wireframe(
    ...
    if (
        not isinstance(n_coils_tor, int)
        or not isinstance(n_coils_pol, int)
        or not isinstance(size_tor, int)
        or not isinstance(size_pol, int)
        or not isinstance(gap_tor, int)
        or not isinstance(gap_pol, int)
    ):
        raise ValueError(
            "n_coils_tor, n_coils_pol, size_tor, size_pol, "
    ...
    for i in range(0, n_coils_tor):
        for j in range(0, n_coils_pol):
            offs_left = (
                i * unit_offs
                + half_gap_tor * n_theta
                - int(0.5 * n_theta)
                + j * unit_pol
                + wframe.n_tor_segments
            )
