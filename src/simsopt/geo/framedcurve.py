...
class FramedCurveFrenet(FramedCurve):
    ...
    def __init__(self, curve, rotation=None):
        ...
        self.binorm = jit(
            lambda gamma, gammadash, gammadashdash, gammadashdashdash, alpha, alphadash: (
                binormal_curvature_pure_frenet(
                    gamma, gammadash, gammadashdash, gammadashdashdash, alpha, alphadash
                )
            )
        )
        self.binormgrad_vjp0 = jit(
            lambda gamma, gammadash, gammadashdash, gammadashdashdash, alpha, alphadash, v: (
                vjp(
                    lambda g: self.binorm(
        ...
        self.torsiongrad_vjp0 = jit(
            lambda gamma, gammadash, gammadashdash, gammadashdashdash, alpha, alphadash, v: (
                vjp(
                    lambda g: self.torsion(
                        g, gammadash, gammadashdash, gammadashdashdash, alpha, alphadash
                    ),
                    gamma,
                )[1](v)[0]
    ...
    def dframe_torsion_by_dcoeff_vjp(self, v):
        ...
        grad0 = self.torsiongrad_vjp0(
            gamma, d1gamma, d2gamma, d3gamma, alpha, alphadash, v
        )
        grad1 = self.torsiongrad_vjp1(
            gamma, d1gamma, d2gamma, d3gamma, alpha, alphadash, v
        )
        grad2 = self.torsiongrad_vjp2(
            gamma, d1gamma, d2gamma, d3gamma, alpha, alphadash, v
        )
        grad3 = self.torsiongrad_vjp3(
            gamma, d1gamma, d2gamma, d3gamma, alpha, alphadash, v
        )
        grad4 = self.torsiongrad_vjp4(
            gamma, d1gamma, d2gamma, d3gamma, alpha, alphadash, v
        )
        grad5 = self.torsiongrad_vjp5(
            gamma, d1gamma, d2gamma, d3gamma, alpha, alphadash, v
        )
    ...
    def dframe_binormal_curvature_by_dcoeff_vjp(self, v):
        ...
        grad0 = self.binormgrad_vjp0(
            gamma, d1gamma, d2gamma, d3gamma, alpha, alphadash, v
        )
        grad1 = self.binormgrad_vjp1(
            gamma, d1gamma, d2gamma, d3gamma, alpha, alphadash, v
        )
        grad2 = self.binormgrad_vjp2(
            gamma, d1gamma, d2gamma, d3gamma, alpha, alphadash, v
        )
        grad3 = self.binormgrad_vjp3(
            gamma, d1gamma, d2gamma, d3gamma, alpha, alphadash, v
        )
        grad4 = self.binormgrad_vjp4(
            gamma, d1gamma, d2gamma, d3gamma, alpha, alphadash, v
        )
        grad5 = self.binormgrad_vjp5(
            gamma, d1gamma, d2gamma, d3gamma, alpha, alphadash, v
        )
...
class FramedCurveCentroid(FramedCurve):
    ...
    def __init__(self, curve, rotation=None):
        ...
        self.torsion = jit(
            lambda gamma, gammadash, gammadashdash, alpha, alphadash: (
                torsion_pure_centroid(gamma, gammadash, gammadashdash, alpha, alphadash)
            )
        )
        self.torsiongrad_vjp0 = jit(
            lambda gamma, gammadash, gammadashdash, alpha, alphadash, v: vjp(
                lambda g: self.torsion(g, gammadash, gammadashdash, alpha, alphadash),
                gamma,
            )[1](v)[0]
        )
        self.torsiongrad_vjp1 = jit(
            lambda gamma, gammadash, gammadashdash, alpha, alphadash, v: vjp(
                lambda g: self.torsion(gamma, g, gammadashdash, alpha, alphadash),
                gammadash,
            )[1](v)[0]
        )
        self.torsiongrad_vjp2 = jit(
            lambda gamma, gammadash, gammadashdash, alpha, alphadash, v: vjp(
                lambda g: self.torsion(gamma, gammadash, g, alpha, alphadash),
                gammadashdash,
            )[1](v)[0]
        )
        self.torsiongrad_vjp4 = jit(
            lambda gamma, gammadash, gammadashdash, alpha, alphadash, v: vjp(
                lambda g: self.torsion(gamma, gammadash, gammadashdash, g, alphadash),
                alpha,
            )[1](v)[0]
        )
        self.torsiongrad_vjp5 = jit(
            lambda gamma, gammadash, gammadashdash, alpha, alphadash, v: vjp(
                lambda g: self.torsion(gamma, gammadash, gammadashdash, alpha, g),
                alphadash,
            )[1](v)[0]
        )
        ...
        self.binorm = jit(
            lambda gamma, gammadash, gammadashdash, alpha, alphadash: (
                binormal_curvature_pure_centroid(
        ...
        self.binormgrad_vjp0 = jit(
            lambda gamma, gammadash, gammadashdash, alpha, alphadash, v: vjp(
                lambda g: self.binorm(g, gammadash, gammadashdash, alpha, alphadash),
                gamma,
            )[1](v)[0]
        )
        self.binormgrad_vjp1 = jit(
            lambda gamma, gammadash, gammadashdash, alpha, alphadash, v: vjp(
                lambda g: self.binorm(gamma, g, gammadashdash, alpha, alphadash),
                gammadash,
            )[1](v)[0]
        )
        self.binormgrad_vjp2 = jit(
            lambda gamma, gammadash, gammadashdash, alpha, alphadash, v: vjp(
                lambda g: self.binorm(gamma, gammadash, g, alpha, alphadash),
                gammadashdash,
            )[1](v)[0]
        )
        self.binormgrad_vjp4 = jit(
            lambda gamma, gammadash, gammadashdash, alpha, alphadash, v: vjp(
                lambda g: self.binorm(gamma, gammadash, gammadashdash, g, alphadash),
                alpha,
            )[1](v)[0]
        )
        self.binormgrad_vjp5 = jit(
            lambda gamma, gammadash, gammadashdash, alpha, alphadash, v: vjp(
                lambda g: self.binorm(gamma, gammadash, gammadashdash, alpha, g),
                alphadash,
            )[1](v)[0]
        )
    ...
    def rotated_frame_dcoeff_vjp(self, v, dn, db, arg=0):
        ...
        zero = np.zeros_like(v)
        if arg == 0:
            return rotated_centroid_frame_dcoeff_vjp0(g, gd, a, (zero, dn * v, db * v))
        if arg == 1:
            return rotated_centroid_frame_dcoeff_vjp1(g, gd, a, (zero, dn * v, db * v))
        if arg == 2:
            return None
        if arg == 3:
            return rotated_centroid_frame_dcoeff_vjp3(g, gd, a, (zero, dn * v, db * v))
...
class FrameRotation(Optimizable):
    def __init__(self, quadpoints, order, scale=1.0, dofs=None):
        ...
        self.jax_alpha = jit(
            lambda dofs, points: jaxrotation_pure(dofs, points, self.order)
        )
        self.jax_alphadash = jit(
            lambda dofs, points: jaxrotationdash_pure(dofs, points, self.order)
        )
...
def rotated_centroid_frame(gamma, gammadash, alpha):
    t = gammadash
    t *= 1.0 / jnp.linalg.norm(gammadash, axis=1)[:, None]
    R = jnp.mean(gamma, axis=0)  # centroid
    delta = gamma - R[None, :]
    n = delta - jnp.sum(delta * t, axis=1)[:, None] * t
    n *= 1.0 / jnp.linalg.norm(n, axis=1)[:, None]
...
def rotated_frenet_frame(gamma, gammadash, gammadashdash, alpha):
    ...
    tdash = (1.0 / jnp.linalg.norm(gammadash, axis=1)[:, None]) ** 2 * (
        jnp.linalg.norm(gammadash, axis=1)[:, None] * gammadashdash
        - (inner(gammadash, gammadashdash) / jnp.linalg.norm(gammadash, axis=1))[
            :, None
        ]
        * gammadash
    )
