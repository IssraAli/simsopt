...
def r_ellipse(a, b, l):
    return (
        ...
        * b
        / jnp.sqrt(
            (b * jnp.cos(2.0 * np.pi * l)) ** 2 + (a * jnp.sin(2.0 * np.pi * l)) ** 2
        )
    )
...
def xyz_cyl(a, b, l):
    ...
    nl = l.size
    out = jnp.zeros((nl, 3))
    out = out.at[:, 0].set(r_ellipse(a, b, l) * jnp.cos(2.0 * np.pi * l))  # x
    out = out.at[:, 1].set(0.0)  # y
    out = out.at[:, 2].set(r_ellipse(a, b, l) * jnp.sin(2.0 * np.pi * l))  # z
    return out
...
def gamma_pure(dofs, points, a, b):
    ...
    g2 = rotations(
        g1, a, b, ypr[0], ypr[1], ypr[2], xyz[0]
    )  # rotate and translate in R
...
class CurvePlanarEllipticalCylindrical(JaxCurve):
    ...
    def __init__(self, quadpoints, a, b, dofs=None):
        ...
        def pure(dofs, points):
            return gamma_pure(dofs, points, a, b)
...
def create_equally_spaced_cylindrical_curves(
    ncurves, nfp, stellsym, R0, a, b, numquadpoints=32
):
    """
