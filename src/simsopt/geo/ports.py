...
class PortSet(object):
    ...
    def __init__(self, ports=None, file=None, port_type=None):
        ...
        if file is not None:
            ...
            if port_type is None:
                raise ValueError(
                    "port_type must be specified if loading " + "from file"
                )
            ...
            if len(file) > 1 and len(port_type) != len(file):
                raise ValueError(
                    "port_type must have one element for every element of file"
                )

            for i in range(len(file)):
                if port_type[i] == "circular" or port_type[i] == CircularPort:
                    self.load_circular_ports_from_file(file[i])
                elif port_type[i] == "rectangular" or port_type[i] == RectangularPort:
    ...
    def add_ports(self, ports):
        ...
        for port in ports:
            if not (
                isinstance(port, CircularPort) or isinstance(port, RectangularPort)
            ):
                raise ValueError(
                    "Element %d of `ports` is not a supported " + "port class"
                )
    ...
    def __add__(self, p):
        ...
        if isinstance(p, PortSet):
            ...
            raise ValueError(
                "Addition with PortSet class instances is only "
    ...
    def load_circular_ports_from_file(self, file):
        ...
        if portdata.shape[1] != 10:
            raise ValueError(
                "Circular ports input file must have 10 columns "
    ...
    def load_rectangular_ports_from_file(self, file):
        ...
        if portdata.shape[1] != 14:
            raise ValueError(
                "Rectangular ports input file must have 17 "
    ...
    def save_ports_to_file(self, fname):
        ...
        if len(circ_ports) > 0:
            lines = [
                ("%.16e," * 9 + "%.16e")
                % (p.ox, p.oy, p.oz, p.ax, p.ay, p.az, p.ir, p.thick, p.l0, p.l1)
                for p in circ_ports
            ]
            all_lines = "\n".join(lines)
    ...
    def collides(self, x, y, z, gap=0.0):
        ...
        if np.isscalar(gap):
            ...
            if len(gaparr) != self.n_ports:
                raise ValueError(
                    "Input gap must be scalar or have one element per port in the set"
                )
    ...
    @SimsoptRequires(
        unstructuredGridToVTK is not None, "to_vtk method requires pyevtk module"
    )
    def to_vtk(self, filename, n_edges=100):
        ...
        for i in range(self.n_ports):
            if isinstance(self.ports[i], CircularPort):
                x_list[i], y_list[i], z_list[i], triangles_list[i] = self.ports[
                    i
                ].mesh_representation(n_edges=n_edges)
            else:
                x_list[i], y_list[i], z_list[i], triangles_list[i] = self.ports[
                    i
                ].mesh_representation()
...
class Port(ABC):
    ...
    def __add__(self, p):
        ...
        if isinstance(p, Port):
            ...
            raise ValueError(
                "Addition with Port class instances is only "
                "supported for Port and PortSet class instances."
            )
...
class CircularPort(Port):
    ...
    def collides(self, x, y, z, gap=0.0):
        ...
        l_proj = (
            (xarr - self.ox) * self.ax
            + (yarr - self.oy) * self.ay
        ...
        in_axial_bounds = np.logical_and(
            l_proj >= l_start - gap, l_proj <= l_stop + gap
        )
    ...
    def mesh_representation(self, n_edges=100):
        ...
        phi = np.linspace(0, 2.0 * np.pi, n_edges, endpoint=False).reshape((1, -1))
        ...
        x = np.concatenate(
            (x0 + ro * rx, x0 + ri * rx, x1 + ri * rx, x1 + ro * rx), axis=0
        ).reshape((-1, 1))
        y = np.concatenate(
            (y0 + ro * ry, y0 + ri * ry, y1 + ri * ry, y1 + ro * ry), axis=0
        ).reshape((-1, 1))
        z = np.concatenate(
            (z0 + ro * rz, z0 + ri * rz, z1 + ri * rz, z1 + ro * rz), axis=0
        ).reshape((-1, 1))
...
class RectangularPort(Port):
    ...
    def __init__(
        ...
        if mod_a == 0 or mod_w == 0:
            raise ValueError(
                "Vectors given by (ax, ay, az),  (wx, wy, wz), "
    ...
    def collides(self, x, y, z, gap=0.0):
        ...
        in_cross_section = np.logical_and(
            np.abs(w_proj) < 0.5 * self.iw + self.thick + gap,
    ...
    def mesh_representation(self):
        ...
        x = np.concatenate(
            (
                x0 + ihw * wxxs + ihh * hxxs,
                x0 + ohw * wxxs + ohh * hxxs,
                x1 + ohw * wxxs + ohh * hxxs,
                x1 + ihw * wxxs + ihh * hxxs,
            ),
            axis=0,
        ).reshape((-1, 1))
        y = np.concatenate(
            (
                y0 + ihw * wyxs + ihh * hyxs,
                y0 + ohw * wyxs + ohh * hyxs,
                y1 + ohw * wyxs + ohh * hyxs,
                y1 + ihw * wyxs + ihh * hyxs,
            ),
            axis=0,
        ).reshape((-1, 1))
        z = np.concatenate(
            (
                z0 + ihw * wzxs + ihh * hzxs,
                z0 + ohw * wzxs + ohh * hzxs,
                z1 + ohw * wzxs + ohh * hzxs,
                z1 + ihw * wzxs + ihh * hzxs,
            ),
            axis=0,
        ).reshape((-1, 1))
