...
class PermanentMagnetGrid:
    ...
    def geo_setup_from_famus(cls, plasma_boundary, Bn, famus_filename, **kwargs):
        ...
        pm_grid.pm_phi = np.arctan2(
            premade_dipole_grid[:, 1], premade_dipole_grid[:, 0]
        )
        uniq_phi, counts_phi = np.unique(
            pm_grid.pm_phi.round(decimals=6), return_counts=True
        )
        ...
        if pol_vectors is not None:
            ...
            if len(pol_vectors.shape) != 3:
                ...
                raise ValueError(
                    "pol_vectors argument can only be used with coordinate_flag = cartesian currently"
                )
            elif pol_vectors.shape[0] != pm_grid.ndipoles:
                raise ValueError(
                    "First dimension of `pol_vectors` array "
    ...
    def geo_setup_between_toroidal_surfaces(
        ...
        **kwargs,
        ...
        pm_grid.pm_phi = np.arctan2(
            pm_grid.dipole_grid_xyz[:, 1], pm_grid.dipole_grid_xyz[:, 0]
        )
        if coordinate_flag == "cylindrical":
            cell_vol = (
                np.sqrt(
                    pm_grid.dipole_grid_xyz[:, 0] ** 2
                    + pm_grid.dipole_grid_xyz[:, 1] ** 2
                )
                * pm_grid.dr
                * pm_grid.dz
                * 2
                * np.pi
                / (pm_grid.nphi * pm_grid.plasma_boundary.nfp * 2)
            )
        else:
    ...
    def _optimization_setup(self):
        ...
        self.A_obj = sopp.dipole_field_Bn(
            ...
            self.plasma_boundary.nfp,
            int(self.plasma_boundary.stellsym),
        ...
        total_error = (
            np.linalg.norm((self.A_obj.dot(self.m0) - self.b_obj), ord=2) ** 2 / 2.0
        )
    ...
    def _print_initial_opt(self):
        ...
        print(
            r"$|b|_2^2 = |B * n|_2^2$ without the permanent magnets = {0:.4e}".format(
                total_Bn
            )
        )
        print(
            r"Initial $|Am_0|_2^2 = |B_M * n|_2^2$ without the coils/plasma = {0:.4e}".format(
                dipole_error
            )
        )
    ...
    def write_to_famus(self, out_dir=""):
        ...
        if self.coordinate_flag == "cartesian":
            ...
            mx = (
                m[:, 0] * cos_ophi * cos_otheta
                - m[:, 1] * sin_ophi
                - m[:, 2] * cos_ophi * sin_otheta
            )
            my = (
                m[:, 0] * sin_ophi * cos_otheta
                + m[:, 1] * cos_ophi
                - m[:, 2] * sin_ophi * sin_otheta
            )
            mz = m[:, 0] * sin_otheta + m[:, 2] * cos_otheta
        ...
        with open(filename, "w") as wfile:
            ...
            for i in range(ndipoles):
                wfile.write(
                    f" 2, {symmetry:1d}, {coilname[i]}, {ox[i]:15.8E}, {oy[i]:15.8E}, "
