...
def get_data(name, **kwargs):
    ...
    **Available configurations:**
    ...
    if cfg == "ncsx":
        ...
        add_default_args(
            kwargs, coil_order=25, magnetic_axis_order=10, points_per_period=10
        )
        ...
        base_curves = CurveXYZFourier.load_curves_from_file(
            filename, order=coil_order, ppp=points_per_period
        )
        nfp = 3

        base_currents = [
            Current(c)
            for c in [6.52271941985300e05, 6.51868569367400e05, 5.37743588647300e05]
        ]
        ...
        add_default_args(
            kwargs, coil_order=16, magnetic_axis_order=10, points_per_period=10
        )
        coil_order = kwargs.pop("coil_order")
        magnetic_axis_order = kwargs.pop("magnetic_axis_order")
        points_per_period = kwargs.pop("points_per_period")
        ...
        nfp = 4
        base_currents = [Current(c) for c in [-1.500725500000000e05] * 6]
        cR = [
            1.221168734647426701e00,
            2.069298947130969735e-01,
            1.819037041932574511e-02,
            4.787659822787012774e-05,
            -3.394778038757981920e-05,
            4.051690884402789139e-05,
            1.066865447680375597e-05,
            -1.418831703321225589e-05,
            2.041664078576817539e-05,
            2.407340923216046553e-05,
            -1.281275289727263035e-05,
            -2.712941403326357315e-05,
            1.828622086757983125e-06,
            1.945955315401206440e-05,
            1.409134021563425399e-05,
            4.572199318143535127e-06,
            3.136573559452139703e-07,
            -3.918158977823491866e-07,
            -2.204187636324686728e-07,
            -4.532041599796651056e-08,
            2.878479243210971143e-08,
            2.102768080992785704e-08,
            -1.267816940685333911e-08,
            -2.268541399245120326e-08,
            -8.015316098897114283e-09,
            6.401201778979550964e-09,
        ]
        sZ = [
            1.670393448410154857e-01,
            1.638250511845155272e-02,
            1.656424673977177490e-04,
            -1.506417857585283353e-04,
            8.367238367133577161e-05,
            -1.386982370447437845e-05,
            -7.536154112897463947e-06,
            -1.533108076767641072e-05,
            -9.966838351213697000e-06,
            2.561158318745738406e-05,
            -1.212668371257951164e-06,
            -1.476513099369021112e-05,
            -3.716380502156798402e-06,
            3.381104573944970371e-06,
            2.605458694352088474e-06,
            5.701177408478323677e-07,
            -1.056254779440627595e-07,
            -1.112799365280694501e-07,
            -5.381768314066269919e-08,
            -1.484193645281248712e-08,
            1.160936870766209295e-08,
            1.466392841646290274e-08,
            1.531935984912975004e-09,
            -6.857347022910395347e-09,
        ...
        add_default_args(
            kwargs,
            coil_order=16,
            magnetic_axis_order=10,
            points_per_period=10,
            length=18,
            nsurfaces=5,
        )
        coil_order, magnetic_axis_order, points_per_period = (
            kwargs.pop("coil_order"),
            kwargs.pop("magnetic_axis_order"),
            kwargs.pop("points_per_period"),
        )
        length, nsurfaces = kwargs.pop("length"), kwargs.pop("nsurfaces")
        filename = THIS_DIR / f"GIULIANI_length{length}_nsurfaces{nsurfaces}"
        base_curves = CurveXYZFourier.load_curves_from_file(
            filename.with_suffix(".curves"), order=coil_order, ppp=points_per_period
        )
        base_currents = [
            Current(c) for c in np.loadtxt(filename.with_suffix(".currents"))
        ]
        ...
        """Get the W7-X coils and magnetic axis."""
        add_default_args(
            kwargs, coil_order=48, magnetic_axis_order=10, points_per_period=2
        )
        coil_order, magnetic_axis_order, points_per_period = (
            kwargs.pop("coil_order"),
            kwargs.pop("magnetic_axis_order"),
        ...
        cR = [
            ...
            0.0161526928867275,
            0.0011820724983052,
            3.43773868380292e-06,
            -4.71423775536881e-05,
            -6.23133271265022e-05,
            2.06622580616597e-06,
            -0.000113256675159501,
            5.0296895894932e-07,
            7.02308687052046e-05,
            5.13479338167885e-05,
        ...
        sZ = [
            ...
            -0.308156954586225,
            -0.0186374002410851,
            -0.000261743895528833,
            5.78207516751575e-05,
            -0.000129121205314107,
            3.08849630026052e-05,
            1.95450172782866e-05,
            -8.32136392792337e-05,
            6.19785500011441e-05,
            1.36521157246782e-05,
            -1.46281683516623e-05,
            -1.5142136543872e-06,
        ]
        ...
        base_curves[6].x = [
            3.850062473963758,
            0.9987505207248398,
            0.049916705720487310,
            0.0012492189452854780,
            1.0408856336378722e-05,
            0,
            0,
            0,
            0,
            0,
            -1.0408856336392461e-05,
            0,
            0,
            -0.9962526034072403,
            -0.049958346351996670,
            -0.0012486983723145407,
            -2.082291883655196e-05,
            0,
            0,
        ]
        ...
        base_curves, base_currents, nfp, coils = download_ID_from_QUASR_database(
            QUASR_ID, return_style="simsopt-style", use_cache=use_cache, verbose=verbose
        )
        bs = BiotSavart(coils)
        ...
        raise ValueError(
            f"Unrecognized configuration name {name!r}; choose from {configurations!r}"
        )
...
def download_ID_from_QUASR_database(
    ID, return_style="simsopt-style", verbose=True, use_cache=True
):
    """
    ...
    if return_style not in ["simsopt-style", "quasr-style"]:
        raise ValueError(
            f"invalid return_style: {return_style}, must be either simsopt-style or quasr-style"
        )

    if requests is None:
        raise ImportError(
            "Requests package is needed for downloading QUASR configurations"
        )
    ...
    if use_cache:
        if os.access(THIS_DIR, os.W_OK):
            FILE_PATH = (
                THIS_DIR / "QUASR_cache" / f"serial{id_str}.json"
            )  # write to cache in simsopt source dir
        elif os.access(os.getcwd(), os.W_OK):
            FILE_PATH = (
                Path(os.getcwd()) / "QUASR_cache" / f"serial{id_str}.json"
            )  # create a local cache
        else:
            raise PermissionError(
                "could not find a writeable location, try again with kwarg use_cache=False"
            )
