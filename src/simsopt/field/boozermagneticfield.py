...
class BoozerAnalytic(BoozerMagneticField):
    ...
    def __init__(
        self, etabar, B0, N, G0, psi0, iota0, Bbar=1.0, I0=0.0, G1=0.0, I1=0.0, K1=0.0
    ):
        self.etabar = etabar
        self.B0 = B0
        self.Bbar = Bbar
    ...
    def _dmodBdzeta_impl(self, dmodBdzeta):
        ...
        r = np.sqrt(np.abs(2 * psi / self.Bbar))
        dmodBdzeta[:, 0] = (
            self.N * self.B0 * self.etabar * r * np.sin(thetas - self.N * zetas)
        )
...
class BoozerRadialInterpolant(BoozerMagneticField):
    ...
    def __init__(
        ...
        if self.mpi is not None:
            ...
            self.dnumnsds_splines = self.mpi.comm_world.bcast(
                self.dnumnsds_splines, root=0
            )
            self.drmncds_splines = self.mpi.comm_world.bcast(
                self.drmncds_splines, root=0
            )
            self.dzmnsds_splines = self.mpi.comm_world.bcast(
                self.dzmnsds_splines, root=0
            )
            self.bmnc_splines = self.mpi.comm_world.bcast(self.bmnc_splines, root=0)
            self.dbmncds_splines = self.mpi.comm_world.bcast(
                self.dbmncds_splines, root=0
            )
            self.d_mn_factor_splines = self.mpi.comm_world.bcast(
                self.d_mn_factor_splines, root=0
            )
            self.mn_factor_splines = self.mpi.comm_world.bcast(
                self.mn_factor_splines, root=0
            )
            self.xm_b = self.mpi.comm_world.bcast(self.xm_b, root=0)
            self.xn_b = self.mpi.comm_world.bcast(self.xn_b, root=0)
            if not self.stellsym:
                self.numnc_splines = self.mpi.comm_world.bcast(
                    self.numnc_splines, root=0
                )
                ...
                self.dnumncds_splines = self.mpi.comm_world.bcast(
                    self.dnumncds_splines, root=0
                )
                self.drmnsds_splines = self.mpi.comm_world.bcast(
                    self.drmnsds_splines, root=0
                )
                self.dzmncds_splines = self.mpi.comm_world.bcast(
                    self.dzmncds_splines, root=0
                )
                self.bmns_splines = self.mpi.comm_world.bcast(self.bmns_splines, root=0)
                self.dbmnsds_splines = self.mpi.comm_world.bcast(
                    self.dbmnsds_splines, root=0
                )
        else:
            self.init_splines()
            if not self.no_K:
    ...
    def compute_K(self):
        ...
        for im in range(len(self.xm_b)):
            ...
            dnumnsds_half[im, :] = (
                ...
                - self.numns_splines[im](self.s_half_ext) * d_mn_factor / mn_factor
            ) / mn_factor
            drmncds_half[im, :] = (
                self.drmncds_splines[im](self.s_half_ext)
                - self.rmnc_splines[im](self.s_half_ext) * d_mn_factor / mn_factor
            ) / mn_factor
            dzmnsds_half[im, :] = (
                self.dzmnsds_splines[im](self.s_half_ext)
                - self.zmns_splines[im](self.s_half_ext) * d_mn_factor / mn_factor
            ) / mn_factor
            ...
            if not self.stellsym:
                dnumncds_half[im, :] = (
                    ...
                    - self.numnc_splines[im](self.s_half_ext) * d_mn_factor / mn_factor
                ) / mn_factor
                drmnsds_half[im, :] = (
                    self.drmnsds_splines[im](self.s_half_ext)
                    - self.rmns_splines[im](self.s_half_ext) * d_mn_factor / mn_factor
                ) / mn_factor
                dzmncds_half[im, :] = (
                    self.dzmncds_splines[im](self.s_half_ext)
                    - self.zmnc_splines[im](self.s_half_ext) * d_mn_factor / mn_factor
                ) / mn_factor
        ...
        if not self.stellsym:
            kmnc_kmns = sopp.compute_kmnc_kmns(
                rmnc_half,
                drmncds_half,
                zmns_half,
                dzmnsds_half,
            ...
            if self.enforce_qs and (self.xn_b[im] != self.N * self.xm_b[im]):
                ...
                if not self.stellsym:
                    self.kmnc_splines.append(
                        InterpolatedUnivariateSpline(
                            self.s_half_ext,
                            self.mn_factor_splines[im](self.s_half_ext) * kmnc[im, :],
                            k=self.order,
                        )
                    )
    ...
    def _K_impl(self, K):
        ...
        sopp.inverse_fourier_transform_odd(
            K[:, 0], kmns, self.xm_b, self.xn_b, thetas, zetas
        )
        if not self.stellsym:
            ...
            sopp.inverse_fourier_transform_even(
                K[:, 0], kmnc, self.xm_b, self.xn_b, thetas, zetas
            )
    ...
    def _dKdtheta_impl(self, dKdtheta):
        ...
        sopp.inverse_fourier_transform_even(
            dKdtheta[:, 0], kmns, self.xm_b, self.xn_b, thetas, zetas
        )
        if not self.stellsym:
            kmnc = np.zeros((len(self.xm_b), len(s)))
            for im in range(len(self.xm_b)):
            ...
            sopp.inverse_fourier_transform_odd(
                dKdtheta[:, 0], kmnc, self.xm_b, self.xn_b, thetas, zetas
            )
    ...
    def _dKdzeta_impl(self, dKdzeta):
        ...
        sopp.inverse_fourier_transform_even(
            dKdzeta[:, 0], kmns, self.xm_b, self.xn_b, thetas, zetas
        )
        if not self.stellsym:
            kmnc = np.zeros((len(self.xm_b), len(s)))
            for im in range(len(self.xm_b)):
            ...
            sopp.inverse_fourier_transform_odd(
                dKdzeta[:, 0], kmnc, self.xm_b, self.xn_b, thetas, zetas
            )
    ...
    def _nu_impl(self, nu):
        ...
        nu[:, 0] = 0.0
        sopp.inverse_fourier_transform_odd(
            nu[:, 0], numns, self.xm_b, self.xn_b, thetas, zetas
        )
        if not self.stellsym:
            ...
            sopp.inverse_fourier_transform_even(
                nu[:, 0], numnc, self.xm_b, self.xn_b, thetas, zetas
            )
    ...
    def _dnudtheta_impl(self, dnudtheta):
        ...
        dnudtheta[:, 0] = 0.0
        sopp.inverse_fourier_transform_even(
            dnudtheta[:, 0], numns, self.xm_b, self.xn_b, thetas, zetas
        )
        if not self.stellsym:
            numnc = np.zeros((len(self.xm_b), len(s)))
            for im in range(len(self.xm_b)):
            ...
            sopp.inverse_fourier_transform_odd(
                dnudtheta[:, 0], numnc, self.xm_b, self.xn_b, thetas, zetas
            )
    ...
    def _dnudzeta_impl(self, dnudzeta):
        ...
        dnudzeta[:, 0] = 0.0
        sopp.inverse_fourier_transform_even(
            dnudzeta[:, 0], numns, self.xm_b, self.xn_b, thetas, zetas
        )
        if not self.stellsym:
            numnc = np.zeros((len(self.xm_b), len(s)))
            for im in range(len(self.xm_b)):
            ...
            sopp.inverse_fourier_transform_odd(
                dnudzeta[:, 0], numnc, self.xm_b, self.xn_b, thetas, zetas
            )
    ...
    def _dnuds_impl(self, dnuds):
        ...
        dnuds[:, 0] = 0.0
        sopp.inverse_fourier_transform_odd(
            dnuds[:, 0], numns, self.xm_b, self.xn_b, thetas, zetas
        )
        if not self.stellsym:
            numnc = np.zeros((len(self.xm_b), len(s)))
            for im in range(len(self.xm_b)):
            ...
            sopp.inverse_fourier_transform_even(
                dnuds[:, 0], numnc, self.xm_b, self.xn_b, thetas, zetas
            )
    ...
    def _dRdtheta_impl(self, dRdtheta):
        ...
        dRdtheta[:, 0] = 0.0
        sopp.inverse_fourier_transform_odd(
            dRdtheta[:, 0], rmnc, self.xm_b, self.xn_b, thetas, zetas
        )
        if not self.stellsym:
            ...
            sopp.inverse_fourier_transform_even(
                dRdtheta[:, 0], rmns, self.xm_b, self.xn_b, thetas, zetas
            )
    ...
    def _dRdzeta_impl(self, dRdzeta):
        ...
        dRdzeta[:, 0] = 0.0
        sopp.inverse_fourier_transform_odd(
            dRdzeta[:, 0], rmnc, self.xm_b, self.xn_b, thetas, zetas
        )
        if not self.stellsym:
            rmns = np.zeros((len(self.xm_b), len(s)))
            for im in range(len(self.xm_b)):
            ...
            sopp.inverse_fourier_transform_even(
                dRdzeta[:, 0], rmns, self.xm_b, self.xn_b, thetas, zetas
            )
    ...
    def _dRds_impl(self, dRds):
        ...
        dRds[:, 0] = 0.0
        sopp.inverse_fourier_transform_even(
            dRds[:, 0], rmnc, self.xm_b, self.xn_b, thetas, zetas
        )
        if not self.stellsym:
            rmns = np.zeros((len(self.xm_b), len(s)))
            for im in range(len(self.xm_b)):
            ...
            sopp.inverse_fourier_transform_odd(
                dRds[:, 0], rmns, self.xm_b, self.xn_b, thetas, zetas
            )
    ...
    def _R_impl(self, R):
        ...
        R[:, 0] = 0.0
        sopp.inverse_fourier_transform_even(
            R[:, 0], rmnc, self.xm_b, self.xn_b, thetas, zetas
        )
        if not self.stellsym:
            rmns = np.zeros((len(self.xm_b), len(s)))
            for im in range(len(self.xm_b)):
            ...
            sopp.inverse_fourier_transform_odd(
                R[:, 0], rmns, self.xm_b, self.xn_b, thetas, zetas
            )
    ...
    def _dZdtheta_impl(self, dZdtheta):
        ...
        dZdtheta[:, 0] = 0.0
        sopp.inverse_fourier_transform_even(
            dZdtheta[:, 0], zmns, self.xm_b, self.xn_b, thetas, zetas
        )
        if not self.stellsym:
            ...
            sopp.inverse_fourier_transform_odd(
                dZdtheta[:, 0], zmnc, self.xm_b, self.xn_b, thetas, zetas
            )
    ...
    def _dZdzeta_impl(self, dZdzeta):
        ...
        dZdzeta[:, 0] = 0.0
        sopp.inverse_fourier_transform_even(
            dZdzeta[:, 0], zmns, self.xm_b, self.xn_b, thetas, zetas
        )
        if not self.stellsym:
            zmnc = np.zeros((len(self.xm_b), len(s)))
            for im in range(len(self.xm_b)):
            ...
            sopp.inverse_fourier_transform_odd(
                dZdzeta[:, 0], zmnc, self.xm_b, self.xn_b, thetas, zetas
            )
    ...
    def _dZds_impl(self, dZds):
        ...
        dZds[:, 0] = 0.0
        sopp.inverse_fourier_transform_odd(
            dZds[:, 0], zmns, self.xm_b, self.xn_b, thetas, zetas
        )
        if not self.stellsym:
            zmnc = np.zeros((len(self.xm_b), len(s)))
            for im in range(len(self.xm_b)):
            ...
            sopp.inverse_fourier_transform_even(
                dZds[:, 0], zmnc, self.xm_b, self.xn_b, thetas, zetas
            )
    ...
    def _Z_impl(self, Z):
        ...
        Z[:, 0] = 0.0
        sopp.inverse_fourier_transform_odd(
            Z[:, 0], zmns, self.xm_b, self.xn_b, thetas, zetas
        )
        if not self.stellsym:
            zmnc = np.zeros((len(self.xm_b), len(s)))
            for im in range(len(self.xm_b)):
            ...
            sopp.inverse_fourier_transform_even(
                Z[:, 0], zmnc, self.xm_b, self.xn_b, thetas, zetas
            )
    ...
    def _modB_impl(self, modB):
        ...
        modB[:, 0] = 0.0
        sopp.inverse_fourier_transform_even(
            modB[:, 0], bmnc, self.xm_b, self.xn_b, thetas, zetas
        )
        if not self.stellsym:
            ...
            sopp.inverse_fourier_transform_odd(
                modB[:, 0], bmns, self.xm_b, self.xn_b, thetas, zetas
            )
    ...
    def _dmodBdtheta_impl(self, dmodBdtheta):
        ...
        dmodBdtheta[:, 0] = 0.0
        sopp.inverse_fourier_transform_odd(
            dmodBdtheta[:, 0], bmnc, self.xm_b, self.xn_b, thetas, zetas
        )
        if not self.stellsym:
            bmns = np.zeros((len(self.xm_b), len(s)))
            for im in range(len(self.xm_b)):
            ...
            sopp.inverse_fourier_transform_even(
                dmodBdtheta[:, 0], bmns, self.xm_b, self.xn_b, thetas, zetas
            )
    ...
    def _dmodBdzeta_impl(self, dmodBdzeta):
        ...
        dmodBdzeta[:, 0] = 0.0
        sopp.inverse_fourier_transform_odd(
            dmodBdzeta[:, 0], bmnc, self.xm_b, self.xn_b, thetas, zetas
        )
        if not self.stellsym:
            bmns = np.zeros((len(self.xm_b), len(s)))
            for im in range(len(self.xm_b)):
            ...
            sopp.inverse_fourier_transform_even(
                dmodBdzeta[:, 0], bmns, self.xm_b, self.xn_b, thetas, zetas
            )
    ...
    def _dmodBds_impl(self, dmodBds):
        ...
        dmodBds[:, 0] = 0.0
        sopp.inverse_fourier_transform_even(
            dmodBds[:, 0], bmnc, self.xm_b, self.xn_b, thetas, zetas
        )
        if not self.stellsym:
            bmns = np.zeros((len(self.xm_b), len(s)))
            for im in range(len(self.xm_b)):
            ...
            sopp.inverse_fourier_transform_odd(
                dmodBds[:, 0], bmns, self.xm_b, self.xn_b, thetas, zetas
            )
