...
class ScaledCurrent(sopp.CurrentBase, CurrentBase):
    ...
    def set_dofs(self, dofs):
        ...
        if dofs.ndim == 0:
            ...
            if len(dofs) != 1:
                raise ValueError(
                    f"ScaledCurrent.set_dofs expects a single DOF, got {len(dofs)}"
                )
...
def load_coils_from_makegrid_file(
    filename, order, ppp=20, group_names=None, regularizations=None
):
    """
    ...
    if regularizations is None:
        ...
        coils = [
            RegularizedCoil(curves[i], Current(currents[i]), regularizations[i])
...
class PSCArray:
    ...
    def __init__(
        ...
        [
            c.fix_all() for c in self.base_psc_currents
        ]  # Fix all the current dofs which are fake anyways
        self.coils = coils_via_symmetries(
            self.base_psc_curves,
            self.base_psc_currents,
            nfp,
            stellsym,
    ...
    def vjp_setup(self, v_currents):
        ...
        vjp_psc = [
            c.dgamma_by_dcoeff_vjp(dJ_dgammas[i])
            + c.dgammadash_by_dcoeff_vjp(dJ_dgammadashs[i])
            for i, c in enumerate(self.psc_curves)
        ]
        vjp_TF = [
            c.vjp(dJ_dgammas2[i], dJ_dgammadashs2[i], dJ_dcurrents2[i])
            for i, c in enumerate(self.coils_TF)
        ]
...
from .psc_bulk import PSCBulkArray, PassiveBulkField, make_bulk_plus_tf_field
