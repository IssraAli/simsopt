...
def _stack_then_device(arrays, dtype=np.double):
    """
    Stack a list of NumPy arrays and move the result to JAX in one call.

    Using ``jnp.array([a, b, ...])`` dispatches one ``array`` primitive per
    row; stacking in NumPy first matches the numeric result and reduces JAX
    ``array`` / ``apply_primitive`` overhead on large dipole runs.
    """
    if len(arrays) == 0:
        return jnp.asarray(np.empty((0,), dtype=dtype))
    return jnp.asarray(np.stack(arrays, axis=0))
...
def _grad_B_at_point_from_coil_set_pure(
    pt, gammas, gammadashs, currents, exclude_index, eps
):
    r"""Return ``(3,3)`` Jacobian :math:`\partial B_i / \partial x_j` at ``pt``.

    Biot--Savart :math:`B` w.r.t. the evaluation point; uses
    :func:`jax.jacobian` of :func:`_B_at_point_from_coil_set_pure`.
    """

    def Bv(x):
        return _B_at_point_from_coil_set_pure(
            x, gammas, gammadashs, currents, exclude_index, eps
        )

    return jax.jacobian(Bv)(pt)


def _A_at_point_from_coil_set_pure(
    pt, gammas, gammadashs, currents, exclude_index, eps
):
    r"""Vector potential from straight segments (closed loops) in ``gammadashs``-free form.

    Uses polygon edges :math:`(\gamma_{j,k}, \gamma_{j,k+1})` of each discretized coil
    and the same scalar prefactor as :data:`Biot_savart_prefactor` in
    :func:`_B_at_point_from_coil_set_pure`.

    The ``gammadashs`` argument is accepted for API symmetry with
    :func:`_B_at_point_from_coil_set_pure` and is not used
    in this straight-segment A implementation.
    """
    n = int(gammas.shape[0])
    npts = int(gammas.shape[1])
    if n == 0:
        return jnp.zeros(3)
    assert gammadashs.shape[0] == n

    def one_seg(p, a, b, ic, do_it):
        d = b - a
        Lm = jnp.linalg.norm(d) + eps
        t = d / Lm
        r1 = jnp.linalg.norm(p - a) + eps
        r2 = jnp.linalg.norm(p - b) + eps
        num = r1 + r2 + Lm
        den = r1 + r2 - Lm
        den = jnp.sign(den) * jnp.maximum(jnp.abs(den), eps)
        seg = t * (Biot_savart_prefactor * ic * jnp.log(jnp.abs(num / den)))
        return do_it * seg
    ...
    def one_coil(ji):
        j = ji
        do_it = jnp.where((jnp.asarray(ex) >= 0) & (j == jnp.asarray(ex)), 0.0, 1.0)
        idx = jnp.arange(npts, dtype=jnp.int32)
        a = gammas[j]
        b = a[jnp.roll(idx, -1)]
        ics = jnp.full((npts,), currents[j], dtype=currents.dtype)
        part = vmap(lambda aa, bb, icc: one_seg(pt, aa, bb, icc, do_it))(a, b, ics)
        return jnp.sum(part, axis=0) / npts

    jidx = jnp.arange(n, dtype=jnp.int32)
    return jnp.sum(vmap(one_coil)(jidx), axis=0)
...
class B2Energy(Optimizable):
    ...
    def __init__(self, target_coils, downsample=1):
        ...
        self.J_jax = jit(
            lambda gammas, gammadashs, currents, downsample: b2energy_pure(
                gammas, gammadashs, currents, downsample, regularizations
            ),
            **args,
        )

        self.dJ_dgammas = jit(
            lambda gammas, gammadashs, currents, downsample: grad(
    ...
    def J(self):
        ...
        """
        args = [
            _stack_then_device([c.curve.gamma() for c in self.target_coils]),
            _stack_then_device([c.curve.gammadash() for c in self.target_coils]),
            jnp.asarray(
                np.array(
                    [c.current.get_value() for c in self.target_coils], dtype=np.double
                )
            ),
            self.downsample,
        ]
    ...
    def dJ(self):
        ...
        vjp = sum(
            [
                c.current.vjp(jnp.asarray([dJ_dcurrents[i]]))
                for i, c in enumerate(self.target_coils)
            ]
        )
        ...
        dJ = (
            sum(
                [
                    c.curve.dgamma_by_dcoeff_vjp(dJ_dgammas[i])
                    for i, c in enumerate(self.target_coils)
                ]
            )
            + sum(
                [
                    c.curve.dgammadash_by_dcoeff_vjp(dJ_dgammadashs[i])
                    for i, c in enumerate(self.target_coils)
                ]
            )
            + vjp
...
class NetFluxes(Optimizable):
    ...
    def __init__(self, target_coil, source_coils, downsample=1):
        ...
        self.J_jax = jit(
            lambda gammadash, A_ext, downsample: net_ext_fluxes_pure(
                gammadash, A_ext, downsample
            ),
            **args,
        )

        self.dJ_dgammadash = jit(
            lambda gammadash, A_ext, downsample: grad(self.J_jax, argnums=0)(
                gammadash, A_ext, downsample
            ),
            **args,
        )
        ...
        self.dJ_dA = jit(
            lambda gammadash, A_ext, downsample: grad(self.J_jax, argnums=1)(
                gammadash, A_ext, downsample
            ),
            **args,
        )
    ...
    def dJ(self):
        ...
        args = [self.target_coil.curve.gammadash(), self.biotsavart.A(), 1]
...
class SquaredMeanForce(Optimizable):
    ...
    def __init__(
        ...
        self.dJ_dgammadash_targets = jit(
            lambda *a: grad(self.J_jax, argnums=2)(*a), **args
        )
        self.dJ_dgammadash_sources = jit(
            lambda *a: grad(self.J_jax, argnums=3)(*a), **args
        )
        self.dJ_dcurrent_targets = jit(
            lambda *a: grad(self.J_jax, argnums=4)(*a), **args
        )
        self.dJ_dcurrent_sources = jit(
            lambda *a: grad(self.J_jax, argnums=5)(*a), **args
        )
        self.dJ_dgamma_sources_fine = jit(
            lambda *a: grad(self.J_jax, argnums=6)(*a), **args
        )
        self.dJ_dgammadash_sources_fine = jit(
            lambda *a: grad(self.J_jax, argnums=7)(*a), **args
        )
        self.dJ_dcurrent_sources_fine = jit(
            lambda *a: grad(self.J_jax, argnums=8)(*a), **args
        )
    ...
    def _J_args(self):
        ...
        if len(self.source_coils_coarse) > 0:
            gammas_coarse = _stack_then_device(
                [c.curve.gamma() for c in self.source_coils_coarse]
            )
            gammadashs_coarse = _stack_then_device(
                [c.curve.gammadash() for c in self.source_coils_coarse]
            )
            currents_coarse = jnp.asarray(
                np.array(
                    [c.current.get_value() for c in self.source_coils_coarse],
                    dtype=np.double,
                )
            )
        ...
        if len(self.source_coils_fine) > 0:
            gammas_fine = _stack_then_device(
                [c.curve.gamma() for c in self.source_coils_fine]
            )
            gammadashs_fine = _stack_then_device(
                [c.curve.gammadash() for c in self.source_coils_fine]
            )
            currents_fine = jnp.asarray(
                np.array(
                    [c.current.get_value() for c in self.source_coils_fine],
                    dtype=np.double,
                )
            )
        return [
    ...
    def dJ(self):
        ...
        if self.psc_array is not None:
            ...
            vjp = sum(
                [
                    c.current.vjp(jnp.asarray([dJ_dcurrent_targets[i]]))
                    for i, c in enumerate(self.target_coils)
                ]
            )
        dJ = (
            sum(
                [
                    c.curve.dgamma_by_dcoeff_vjp(dJ_dgamma_targets[i])
                    for i, c in enumerate(self.target_coils)
                ]
            )
            + sum(
                [
                    c.curve.dgammadash_by_dcoeff_vjp(dJ_dgammadash_targets[i])
                    for i, c in enumerate(self.target_coils)
                ]
            )
            + vjp
        ...
        if len(self.source_coils_coarse) > 0:
            dJ += (
                sum(
                    [
                        c.curve.dgamma_by_dcoeff_vjp(dJ_dgamma_coarse[i])
                        for i, c in enumerate(self.source_coils_coarse)
                    ]
                )
                + sum(
                    [
                        c.curve.dgammadash_by_dcoeff_vjp(dJ_dgammadash_coarse[i])
                        for i, c in enumerate(self.source_coils_coarse)
                    ]
                )
                + sum(
                    [
                        c.current.vjp(jnp.asarray([dJ_dcurrent_coarse[i]]))
                        for i, c in enumerate(self.source_coils_coarse)
                    ]
                )
            )
        if len(self.source_coils_fine) > 0:
            dJ += (
                sum(
                    [
                        c.curve.dgamma_by_dcoeff_vjp(dJ_dgamma_fine[i])
                        for i, c in enumerate(self.source_coils_fine)
                    ]
                )
                + sum(
                    [
                        c.curve.dgammadash_by_dcoeff_vjp(dJ_dgammadash_fine[i])
                        for i, c in enumerate(self.source_coils_fine)
                    ]
                )
                + sum(
                    [
                        c.current.vjp(jnp.asarray([dJ_dcurrent_fine[i]]))
                        for i, c in enumerate(self.source_coils_fine)
                    ]
                )
            )
...
class LpCurveForce(Optimizable):
    ...
    def __init__(
        ...
        self.J_jax = jit(
            lambda gammas_targets, gammas_coarse, gammadashs_targets, gammadashs_coarse, gammadashdashs_targets, currents_targets, currents_coarse, gammas_fine, gammadashs_fine, currents_fine, downsample: (
                lp_force_pure(
                    gammas_targets,
                    gammas_coarse,
                    gammadashs_targets,
    ...
    def _J_args(self):
        ...
        if len(self.source_coils_fine) > 0:
            ...
            gammadashs_fine = jnp.array(
                [c.curve.gammadash() for c in self.source_coils_fine]
            )
            currents_fine = jnp.array(
                [c.current.get_value() for c in self.source_coils_fine]
            )
        gammas_coarse = (
            jnp.array([c.curve.gamma() for c in self.source_coils_coarse])
            if len(self.source_coils_coarse) > 0
            else jnp.zeros((0, 1, 3))
        )
        gammadashs_coarse = (
            jnp.array([c.curve.gammadash() for c in self.source_coils_coarse])
            if len(self.source_coils_coarse) > 0
            else jnp.zeros((0, 1, 3))
        )
        currents_coarse = (
            jnp.array([c.current.get_value() for c in self.source_coils_coarse])
            if len(self.source_coils_coarse) > 0
            else jnp.zeros((0,))
        )
        return [
    ...
    def dJ(self):
        ...
        dJ = (
            ...
            + sum(
                [
                    c.curve.dgammadashdash_by_dcoeff_vjp(dJ_dgammadashdash_targets[i])
                    for i, c in enumerate(self.target_coils)
                ]
            )
            + vjp
...
class SquaredMeanTorque(Optimizable):
    ...
    def _J_args(self):
        ...
        currents_coarse = jnp.zeros((0,))
        if len(self.source_coils_coarse) > 0:
            gammas_coarse = jnp.array(
                [c.curve.gamma() for c in self.source_coils_coarse]
            )
            gammadashs_coarse = jnp.array(
                [c.curve.gammadash() for c in self.source_coils_coarse]
            )
            currents_coarse = jnp.array(
                [c.current.get_value() for c in self.source_coils_coarse]
            )
        gammas_fine = jnp.zeros((0, 1, 3))
        gammadashs_fine = jnp.zeros((0, 1, 3))
