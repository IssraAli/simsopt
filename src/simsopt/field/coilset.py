...
class CoilSet(Optimizable):
    ...
    def __init__(self, base_coils=None, coils=None, surface=None):
        ...
        if surface is None:
            self._surface = SurfaceRZFourier().copy(
                range=SurfaceRZFourier.RANGE_HALF_PERIOD
            )
        # stellarator symmetric surfaces are more efficiently evaluated on a half-period
            ...
            if surface.stellsym:
                if surface.deduced_range is surface.RANGE_HALF_PERIOD:
                    ...
                    newsurf = surface.to_RZFourier().copy(
        ...
        if base_coils is not None:
            ...
            if coils is None:
                self.coils = coils_via_symmetries(
                    ...
                    stellsym=self._surface.stellsym,
                )
            else:
                self.coils = coils
        else:
            ...
            base_curves = self._circlecurves_around_surface(
                self._surface, coils_per_period=10
            )
            ...
            for current in base_currents:
                current.fix_all()
            base_coils = [
                Coil(curv, curr) for (curv, curr) in zip(base_curves, base_currents)
            ]
            self._base_coils = base_coils
            self.coils = coils_via_symmetries(
                [coil.curve for coil in base_coils],
    ...
    def for_surface(
        ...
        **kwargs,
        ...
        base_curves = CoilSet._circlecurves_around_surface(
            surf, coils_per_period=coils_per_period, **kwargs
        )
        ...
        if current_constraint == "fix_one":
            ...
            raise ValueError(
                "current_constraint must be 'fix_one', 'fix_all' or 'free_all'"
            )
        base_coils = [
            Coil(curv, curr) for (curv, curr) in zip(base_curves, base_currents)
        ]
    ...
    def base_coils(self, base_coils):
        ...
        self._base_coils = base_coils
        self._coils = coils_via_symmetries(
            [coil.curve for coil in base_coils],
            [coil.current for coil in base_coils],
            nfp=self._surface.nfp,
            stellsym=self._surface.stellsym,
    ...
    @staticmethod
    def _circlecurves_around_surface(
        surf,
        coils_per_period=4,
        order=6,
        R0=None,
        ...
        if R1 is None:
            ...
            R1 = (
                np.sqrt(
                    surf.to_RZFourier().get_rc(1, 0) ** 2
                    + surf.to_RZFourier().get_zs(1, 0) ** 2
                )
                * factor
            )
        return create_equally_spaced_curves(
            coils_per_period, nfp, stellsym=use_stellsym, R0=R0, R1=R1, order=order
        )
    ...
    def get_dof_orders(self):
        ...
        orders = np.zeros(
            self.dof_size
        )  # dofs which are not Fourier coefficients are treated as zeroth' order.
    ...
    def length_penalty(self, TOTAL_LENGTH, f):
        ...
        lenth_optimizable = (
            sum(CurveLength(coil.curve) for coil in self.base_coils)
    ...
    def cc_distance_penalty(self, DISTANCE_THRESHOLD):
        ...
        target = CurveCurveDistance(
            curves, DISTANCE_THRESHOLD, num_basecurves=len(self.base_coils)
        )
        # make self parent as self's dofs will propagate to coils
        for parent in target.parents:
            target.remove_parent(parent)
    ...
    def lp_curvature_penalty(self, CURVATURE_THRESHOLD, p=2):
        ...
        target = sum(
            LpCurveCurvature(curve, p, CURVATURE_THRESHOLD) for curve in base_curves
        )
        # make self parent as self's dofs will propagate to coils
        for parent in target.parents:
            target.remove_parent(parent)
    ...
    def meansquared_curvature_threshold(self, CURVATURE_THRESHOLD):
        ...
        Return a penalty function on the mean squared curvature of the coils
        """
        meansquaredcurvatures = [
            MeanSquaredCurvature(coil.curve) for coil in self.base_coils
        ]
        target = sum(
            QuadraticPenalty(msc, CURVATURE_THRESHOLD, "max")
            for msc in meansquaredcurvatures
        )
        for parent in target.parents:
            target.remove_parent(parent)
        target.append_parent(self)
    ...
    def total_length(self):
        ...
        multiplicity = len(self.coils) / len(self.base_coils)
        lengthoptimizable = (
            sum(CurveLength(coil.curve) for coil in self.base_coils) * multiplicity
        )
    ...
    def to_vtk(self, filename, add_biotsavart=True, close=False):
        ...
        """
        curves_to_vtk(
            [coil.curve for coil in self.coils], filename + "_coils", close=close
        )
            ...
            if add_biotsavart:
                pointData = {
                    "B_N": np.sum(
                        self.bs.B().reshape(
                            (
                                len(self.surface.quadpoints_phi),
                                len(self.surface.quadpoints_theta),
                                3,
                            )
                        )
                        * self.surface.unitnormal(),
                        axis=2,
                    )[:, :, None]
                }
            else:
    ...
    def plot(self, **kwargs):
        ...
        self.surface.to_RZFourier().copy(range=SurfaceRZFourier.RANGE_FULL_TORUS).plot(
            **kwargs, show=True
        )
...
class ReducedCoilSet(CoilSet):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if (
            coilset is None
        ):  # need 'is' (idenity comparison), overloaded __eq__ fails here.
            coilset = CoilSet()
        if nsv is None:
            nsv = coilset.dof_size

        # raise error if any of [s_diag, u_matrix, vh_matrix] are None but not all of them are:
        if any([s_diag is None, u_matrix is None, vh_matrix is None]) and not all(
            [s_diag is None, u_matrix is None, vh_matrix is None]
        ):
            raise ValueError(
                "If any of [s_diag, u_matrix, vh_matrix] are None, all must be None"
            )

        if s_diag is None:
            if nsv > coilset.dof_size:
                raise ValueError(
                    "nsv must be equal to or smaller than the coilset's number of DOFs if initializing with None"
                )
            s_diag = np.ones(nsv)
            u_matrix = np.eye(nsv)
        ...
        self._coil_x0 = np.copy(
            self.coilset.x
        )  # DID NOT COPY BEFORE, CHECK CONVERGENCE
        self._s_diag = s_diag
        if nsv > len(s_diag):
            raise ValueError(
                "nsv must be equal to or smaller than the number of singular values"
            )
        self._nsv = nsv
        Optimizable.__init__(
            self,
            x0=np.zeros_like(self._s_diag[:nsv]),
            names=self._make_names(),
            external_dof_setter=ReducedCoilSet.set_dofs,
            **kwargs,
        )
    ...
    def coils(self, *args, **kwargs):
        raise ValueError(
            "You cannot change the coils of a ReducedCoilSet. Change the base_coils instead."
        )
    ...
    def set_dofs(self, x):
        ...
        padx = np.pad(
            x, (0, len(self._coil_x0) - self.nsv), mode="constant", constant_values=0
        )  # create zero-padded vector of length of coil DOFs.
        self.coilset.x = (
            self._coil_x0 + (padx) @ self._vh_matrix
        )  # translate reduced vectors amplitude to movement of coils.

    def plot_singular_vector(
        self, n, eps=1e-4, show_delta_B=True, engine="mayavi", show=False, **kwargs
    ):
        """
        ...
        if engine == "mayavi":
            ...
            startpositions = [
                np.copy(coil.curve.gamma()) for coil in self.coilset.coils
            ]
            plot(
                self.coilset.coils,
                close=True,
            ...
            if show_delta_B:
                startB = np.copy(
                    np.sum(
                        bs.B().reshape(
                            (
                                plotsurf.quadpoints_phi.size,
                                plotsurf.quadpoints_theta.size,
                                3,
                            )
                        )
                        * plotsurf.unitnormal()
            ...
            raise ValueError(
                "plotting a ReducedCoilSet is only implemented in Mayavi. you can access and plot the surface and coils attributes directly."
            )
