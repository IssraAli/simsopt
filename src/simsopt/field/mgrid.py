...
class MGrid:
    ...
    def write(self, filename):
        ...
        with netcdf_file(filename, "w", mmap=False, version=2) as ds:
            ...
            if self.n_ext_cur == 1:
                ...
                var_coil_group = ds.createVariable(
                    "coil_group",
                    "c",
                    (
            ...
            var_raw_coil_cur = ds.createVariable(
            ...
            for j in np.arange(self.n_ext_cur):
                ...
                if len(self.ar_arr) > 0:
                    var_ar_001 = ds.createVariable(
                        "ar" + tag, "f8", ("phi", "zee", "rad")
                    )
                    var_ar_001[:, :, :] = self.ar_arr[j]
                if len(self.ap_arr) > 0:
                    var_ap_001 = ds.createVariable(
                        "ap" + tag, "f8", ("phi", "zee", "rad")
                    )
                    var_ap_001[:, :, :] = self.ap_arr[j]
                if len(self.az_arr) > 0:
                    var_az_001 = ds.createVariable(
                        "az" + tag, "f8", ("phi", "zee", "rad")
                    )
    ...
    def from_file(cls, filename):
        ...
        with netcdf_file(filename, "r", mmap=False, version=2) as f:
            ...
            if len(f.variables["coil_group"].dimensions) == 2:
                mgrid.coil_names = [
                    _unpack(coil_data[j]) for j in range(mgrid.n_ext_cur)
                ]
            else:
    ...
    def plot(self, jphi=0, bscale=0, show=True):
        ...
        def subplot_slice(s, data, rax, zax, bscale=0.3, tag=""):
            ...
            if bscale > 0:
                cf = axs[s].contourf(
                    rax, zax, data, np.linspace(-bscale, bscale, 11), extend="both"
                )
            else:
