...
class MagneticField(sopp.MagneticField, Optimizable):
    ...
    def set_points_cart(self, xyz):
        if len(xyz.shape) != 2 or xyz.shape[1] != 3:
            raise ValueError(
                f"xyz array should have shape (n, 3), but has shape {xyz.shape}"
            )
        if not xyz.flags["C_CONTIGUOUS"]:
            raise ValueError(
                "xyz array should be C contiguous. Consider using `numpy.ascontiguousarray`."
            )
    ...
    def set_points_cyl(self, rphiz):
        if len(rphiz.shape) != 2 or rphiz.shape[1] != 3:
            raise ValueError(
                f"rphiz array should have shape (n, 3), but has shape {rphiz.shape}"
            )
        if not rphiz.flags["C_CONTIGUOUS"]:
            raise ValueError(
                "rphiz array should be C contiguous. Consider using `numpy.ascontiguousarray`."
            )
    ...
    def recompute_bell(self, parent=None):
        """
        Invalidate the cached magnetic-field properties when any DOF in this
        object's unique ancestor lineage is free.

        Performance note: this method is called once per ``Optimizable`` node
        each time any DOF anywhere in the graph changes (e.g. from
        ``JF.x = dofs`` during optimization). For dipole-array problems with
        many ``BiotSavart`` / ``MagneticFieldSum`` / ``MagneticFieldMultiply``
        nodes this can fire hundreds of thousands of times per optimizer
        iteration, so the implementation must be O(1) in the steady state.

        The previous implementation called ``np.any(self.dofs_free_status)``
        which rebuilt a concatenated boolean array on every call and
        dominated runtime profiles. We now read the cached
        ``Optimizable._any_free_in_lineage`` boolean, which is only
        recomputed on fix/unfix or structural changes.

        Additionally, for each unique ``Dofs`` in the lineage we compare a
        monotonic ``Dofs._state_version`` to the value seen the last time
        this field flushed its C++ cache. If the dictionary of versions is
        unchanged, the native ``invalidate_cache`` is skipped. This covers
        repeated ``set_recompute_flag``/``recompute_bell`` visits at the
        same DOF values (e.g. SciPy line-search re-tries with identical
        ``x``).

        Parameters
        ----------
        parent : Optimizable, optional
            The parent that triggered the recompute (unused, kept for API
            compatibility with ``Optimizable.set_recompute_flag``).
        """
        if not self._any_free_in_lineage:
            return
        current = {id(o._dofs): o._dofs._state_version for o in self._unique_dof_opts}
        last = getattr(self, "_mf_last_seen_dof_versions", None)
        if last is not None and last == current:
            return
        # Copy so later mutation of a dict on another object cannot alias.
        self._mf_last_seen_dof_versions = {k: v for k, v in current.items()}
    ...
    def to_vtk(
        self, filename, nr=10, nphi=10, nz=10, rmin=1.0, rmax=2.0, zmin=-0.5, zmax=0.5
    ):
        ...
        gridToVTK(
            ...
            Z,
            pointData={
                "B": (contig(vals[..., 0]), contig(vals[..., 1]), contig(vals[..., 2]))
            },
        )
    ...
    def to_mgrid(
        ...
        mgrid = MGrid(
            nfp=nfp, nr=nr, nz=nz, nphi=nphi, rmin=rmin, rmax=rmax, zmin=zmin, zmax=zmax
        )
        if include_potential:
            mgrid.add_field_cylindrical(
                br_3, bp_3, bz_3, ar=ar_3, ap=ap_3, az=az_3, name="simsopt_coils"
            )
        else:
