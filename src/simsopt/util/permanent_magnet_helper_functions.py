...
def initialize_coils_for_pm_optimization(config_flag, TEST_DIR, s, out_dir=""):
    ...
    if "muse" in config_flag:
        ...
        for i in range(ncoils):
            coils.append(
                RegularizedCoil(
                    base_curves[i], base_currents[i], regularization_circ(0.05)
                )
            )
        ...
        base_curves = create_equally_spaced_curves(
            ncoils, s.nfp, stellsym=True, R0=R0, R1=R1, order=order, numquadpoints=64
        )
        base_currents = [
            (Current(total_current / ncoils * 1e-5) * 1e5) for _ in range(ncoils - 1)
        ]
        ...
        coils = coils_via_symmetries(
            base_curves, base_currents, s.nfp, True, regularizations
        )
...
def make_optimization_plots(RS_history, m_history, m_proxy_history, pm_opt, out_dir=""):
    ...
    plt.figure()
    m0_abs = (
        np.sqrt(np.sum(pm_opt.m.reshape(pm_opt.ndipoles, 3) ** 2, axis=-1))
        / pm_opt.m_maxima
    )
    mproxy_abs = (
        np.sqrt(np.sum(pm_opt.m_proxy.reshape(pm_opt.ndipoles, 3) ** 2, axis=-1))
        / pm_opt.m_maxima
    )
    ...
    if len(RS_history) != 0:
        ...
        m_history = m_history.reshape(
            m_history.shape[0] * m_history.shape[1], pm_opt.ndipoles, 3
        )
        m_proxy_history = np.array(m_proxy_history).reshape(
            m_history.shape[0], pm_opt.ndipoles, 3
        )
        ...
        for i, datum in enumerate([m_history, m_proxy_history]):
            ...
            def prepare_animation(bar_container):
                def animate(frame_number):
                    ...
                    data = (
                        np.sqrt(np.sum(datum[frame_number, :] ** 2, axis=-1))
                        / pm_opt.m_maxima
                    )
            ...
            if rho is not None:
                ax.hist(
                    abs(rho), bins=np.linspace(0, 1, 40), log=True, alpha=0.6, color="g"
                )
            _, _, bar_container = ax.hist(
                data, bins=np.linspace(0, 1, 40), log=True, alpha=0.6, color="r"
            )
...
def run_Poincare_plots_with_permanent_magnets(
    s_plot, bs, b_dipole, comm, filename_poincare, out_dir=""
):
    """
...
def initialize_default_kwargs(algorithm="RS"):
    ...
    if algorithm == "RS":
        ...
        kwargs["max_iter_RS"] = (
            2  # Number of total iterations of the relax-and-split algorithm
        )
        kwargs["reg_l2"] = 0.0
