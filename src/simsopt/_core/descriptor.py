...
class OneOf(Validator):
    ...
    def validate(self, value):
        if value not in self.options:
            raise ValueError(
                f"{value!r} is not a valid option for {self.public_name!r}.\n"
...
class _Real(Validator):
    ...
    def validate(self, value):
        if not isinstance(value, numbers.Real):
            raise TypeError(f"Expected {value!r} should be of Real type")
        super().validate(value)
...
class _Integral(Validator):
    ...
    def validate(self, value):
        if not isinstance(value, numbers.Integral):
            raise TypeError(f"Expected {value!r} should be of Integer type")
        super().validate(value)
