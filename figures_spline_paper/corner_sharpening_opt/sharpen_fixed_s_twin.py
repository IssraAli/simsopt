...
def _corner_pair_geometry(
    ...
    for i, cs in enumerate(spline_surf.cs_list):
        ...
        u0, u1 = _find_both_corner_us(
            samples, corner_criterion, z_embed_list[i]
        )
        geometry.append(
            [
...
def _apply_crossover_leg(cs, g, replace_window, u_stop, target, apex_idx):
    ...
    for idx in free_idx:
        if idx == apex_idx:
            continue
        s_idx = s_lo_abs + (
            (_u_to_arclength(greville[idx], us, s, L) - s_lo_abs) % L
        )
        frac = (
            0.0
            if s_hi_abs <= s_lo_abs
            else (s_idx - s_lo_abs) / (s_hi_abs - s_lo_abs)
        )
        ...
        cs.set(f"r_{j}", r_new)
        pinned_theta = j == 0 or (
            cs.z_sym and cs.n_ctrl_pts % 2 == 0 and j == n_pts - 1
        )
...
def _crossover_stops_per_cs(geometry, d_fraction, keep_between):
    ...
    for cs_corners in geometry:
        ...
        stops.append(
            (c0_replace, c0_target, c0_u_stop, c1_replace, c1_target, c1_u_stop)
        )
    return stops
...
def sharpen_with_crossover(
    ...
    """
    geometry = _corner_pair_geometry(
        spline_surf, s1, s2, corner_criterion, n_samples
    )
    spline_surf.fix_all()
    ...
    for i, cs_corners in enumerate(geometry):
        c0, c1 = cs_corners
        samples = c0["samples"]
        r_between = _arc_mean_lab_radius(
            samples, c0["u_plus"], c1["u_minus"], r_embed_list[i]
        )
        r_around = _arc_mean_lab_radius(
            samples, c1["u_plus"], c0["u_minus"], r_embed_list[i]
        )
    ...
    def _build_twin(keep_between, ntheta):
        ...
        twin = SurfaceBSpline(
            **spline_kwargs, quadpoints_theta=quadpoints_theta
        )
        ...
        for cs, cs_corners, (
            ...
            c0, c1 = cs_corners
            _apply_crossover_leg(
                cs, c0, c0_replace, c0_u_stop, c0_target, _apex_full_index(c0)
            )
            _apply_crossover_leg(
                cs, c1, c1_replace, c1_u_stop, c1_target, _apex_full_index(c1)
            )
        return twin
    ...
    surf_outboard, surf_inboard = (
        (surf_between, surf_around)
        if outboard_is_between
        else (surf_around, surf_between)
    )
    return surf_outboard, surf_inboard, geometry, outboard_is_between


def pickle_twin_surface(
    surf, spline_kwargs, filename, refine_poloidal_count=REFINE_POLOIDAL_COUNT
):
    """
...
def _cs_curve_xy(surf, i, us=None):
    ...
    pos, _, _ = _periodic_curve_deriv2(
        core, cs.get_w_ctrl_full(), surf.p_u, surf.knot_parametrization, us
    )
    return pos
...
def interpolated_leg_u_bounds(
    spline_surf, geometry, leg_fraction, keep_between, phis
):
    """
    ...
    u_los, u_his = (
        (c0_u_stops, c1_u_stops) if keep_between else (c1_u_stops, c0_u_stops)
    )
...
def compute_uphi_grid_interpolated(
    spline_surf, geometry, leg_fraction, keep_between, phis, ntheta=60
):
    """
    ...
    theta.reshape(-1)).
    """
    u_lo_phi, u_hi_phi = interpolated_leg_u_bounds(
        spline_surf, geometry, leg_fraction, keep_between, phis
    )
    phi_rows, theta_rows = [], []
...
def build_sharpened_twins(
    ...
    if corner_criterion not in ("z", "curvature"):
        raise ValueError(
            f"corner_criterion must be 'z' or 'curvature', got {corner_criterion!r}"
        )
...
def plot_leg_length_grid(
    ...
    for surf, keep_between, color, label in (
        ...
        grid = compute_uphi_grid_interpolated(
            spline_surf, geometry, leg_fraction, keep_between, phis, ntheta
        )
...
def plot_before_after_twin(
    ...
    for i, ax in enumerate(axs):
        ...
        curve_before, _ = spline_surf_before.cross_section_xy(
            cs_before, n_samples=300
        )
        ...
        for surf, keep_between, color, label in (
            ...
            u_lo, u_hi = interpolated_leg_u_bounds(
                spline_surf, geometry, leg_fraction, keep_between, phi_i
            )
            us = _leg_u_row(u_lo[0], u_hi[0], keep_between, ntheta)
            pos = _cs_curve_xy(surf, i, us=us)
            ax.plot(
                pos[:, 0], pos[:, 1], color=color, lw=2, alpha=0.8, label=label
            )
...
if __name__ == "__main__":
    ...
    pickle_twin_surface(
        surf_outboard, spline_kwargs, "sharpen_fixed_s_twin_outboard.pkl"
    )
    pickle_twin_surface(
        surf_inboard, spline_kwargs, "sharpen_fixed_s_twin_inboard.pkl"
    )
    proc0_print(
    ...
    proc0_print("")
    proc0_print(
        "End of figures_spline_paper/corner_sharpening/sharpen_fixed_s_twin.py"
    )
