...
import numpy as np
from simsopt.util.spline_helpers import b_p_deriv2, chord_length_knots, uniform_knots
...
def _periodic_curve_deriv2(core, weights, p, knot_parametrization, u, domain=2 * np.pi):
    r"""
...
def _greville_abscissae(m, p, knot_parametrization, core, domain=2 * np.pi):
    ...
    greville = np.array([np.mean(t[p + i + 1 : 2 * p + i + 1]) for i in range(m)])
...
def _corner_geometry_from_u(samples, u_corner, s1, s2):
    ...
    opening_angle = np.arccos(
        np.clip(
            np.dot(d_minus, d_plus) / (np.linalg.norm(d_minus) * np.linalg.norm(d_plus)),
            -1.0,
            1.0,
        )
    )
...
def tangent_extension_geometry(spline_surf, s1, s2, corner_criterion, n_samples=4000):
    """
    ...
    for i, cs in enumerate(spline_surf.cs_list):
        ...
        us_corner = _find_corner_us(samples, corner_criterion, cs.z_sym, z_embed_list[i])
        cs_corners = [
            _corner_geometry_from_u(samples, u_corner, s1, s2) for u_corner in us_corner
        ]
...
def _apply_tangent_extension(spline_surf, geometry):
    ...
    for cs, cs_corners in zip(spline_surf.cs_list, geometry):
        ...
        for g in cs_corners:
            ...
            corner_rel = (u_corner - u_minus) % (2 * np.pi)
            apex_full_idx = int(free_idx[np.argmin(np.abs(rel[free_idx] - corner_rel))])
            ...
            for idx in free_idx:
                ...
                if idx == apex_full_idx:
                    ...
                    frac = (
                        ...
                        else (s_idx - s_minus_abs) / (s_corner_abs - s_minus_abs)
                    )
                    ...
                    frac = (
                        ...
                        else (s_idx - s_corner_abs) / (s_plus_abs - s_corner_abs)
                    )
...
def plot_tangent_extension(
    ...
    for ax, cs, cs_corners in zip(axs, spline_surf.cs_list, geometry):
        ...
        for k, g in enumerate(cs_corners):
            ...
            ax.scatter(
                *apex, c="tab:red", marker="*", s=120, zorder=3, label=apex_label
            )
...
if __name__ == "__main__":
    ...
    fig, axs = plt.subplots(1, spline_surf.n_cs, squeeze=False, figsize=(4 * spline_surf.n_cs, 4))
    for ax, cs in zip(axs[0], spline_surf.cs_list):
        ...
        ax.plot(ctrl_closed[:, 0], ctrl_closed[:, 1], ls="--", marker="o", alpha=0.4)
        ax.set_aspect("equal")
