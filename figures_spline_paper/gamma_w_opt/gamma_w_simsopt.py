...
class _RawBoozerBase:
    def __init__(self, bx):
        ...
        # Boozer_G_all/Boozer_I_all/iota are on VMEC's full half-grid, not
        # restricted to the registered/computed surfaces in s_b -- must be
        # indexed down via compute_surfs to line up with s_b for the spline.
        # (Boozer_G/Boozer_I are already restricted to compute_surfs.)
        compute_surfs = np.asarray(bx.compute_surfs, int)
        self._G_sp = CubicSpline(s_b, np.asarray(bx.Boozer_G, float))
        self._I_sp = CubicSpline(s_b, np.asarray(bx.Boozer_I, float))
        self._io_sp = CubicSpline(s_b, np.asarray(bx.iota, float)[compute_surfs])
