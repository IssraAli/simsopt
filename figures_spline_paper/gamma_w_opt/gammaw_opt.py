...
# ==============================================================
# Profiles (density, temperature, pressure, current)
# ==============================================================
beta = 0.025
ne0 = 3e20 * (beta / 0.05) ** (1 / 3)
Te0 = 15e3 * (beta / 0.05) ** (2 / 3)
Zeff = 1.0

ne = ProfilePolynomial(ne0 * np.array([1.04, 0, 0, 0, 0, -1.0]))
Te = ProfilePolynomial(Te0 * np.array([1.04, -1.0]))
ni = ne
Ti = Te
...
n_spline = 3 * 2 + 5  # current spline representation
s_spline = np.linspace(0, 1, n_spline)
current = ProfileSpline(s_spline, s_spline * (1 - s_spline) * 4)
current.fix_all()
current.unfix_all()
current.fix(0)
current.fix(n_spline - 1)
current.fix(int((n_spline - 1) / 2))
...
vmec.unfix("phiedge")
# Configure gamma_W objective

booz = Boozer(vmec, mpol=12, ntor=12)
# Register surfaces before vmec has ever run, so compute the half-grid
# manually from ns rather than reading vmec.s_half_grid (which is only
# populated as a side effect of vmec.run()).
ns = 50
s_full = np.linspace(0, 1, ns)
ds = s_full[1] - s_full[0]
s_half = s_full[1:] - 0.5 * ds
booz.register(s_half)


def GammaWCallable(booz):
    booz.run()
    # simsopt.mhd.Boozer.run() does not forward VMEC's full-grid toroidal
    # flux profile into booz_xform's init_from_vmec, so bx.phi is left
    # empty; gamma_w_from_boozer needs it to compute psi0.
    booz.bx.phi = booz.equil.wout.phi
    return gamma_w_from_boozer(booz)["Gamma_W"]


gamma_W = make_optimizable(GammaWCallable, booz)
...
prob = LeastSquaresProblem.from_tuples(
    [
        (gamma_W.J, 0, 1),
        (vmec.aspect, 5, 100),
        (vmec.iota_edge, 0.42, 100),
        (vmec.iota_axis, 0.42, 100),
        (vmec.mean_iota, 0.42, 100),
    ]
)

vmec.run()
proc0_print("Initial gamma_W:", np.sum(gamma_W.J() ** 2))
