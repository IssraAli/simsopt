...
#
# Copied from simsopt-pyoculus-manifold-opt/field_topology_optimizables.py into
# da_remix (see manifold_opt_twin.py) -- only the relative imports below were
# changed (this file no longer lives inside the simsopt.mhd package), plus one
# new class, TargetSurfaceDeviation, appended at the end. Everything above
# that point is Chris Smiet's original.
...
__all__ = [
    'PyOculusFixedPoint', 'ClinicConnection', 'SimpleFixedPoint_RZ', 'SimpleIntegrator',
    'TargetSurfaceDeviation',
]
...
class PyOculusFixedPoint(Optimizable):
    ...
    def __init__(self, field: "MagneticField",
        ...
        # THe below throws error for 1d solvers
...
def _point_to_polyline_min_dist(point_rz, polyline_rz):
    """
    Min Euclidean (R, Z) distance from `point_rz` to the closed
    piecewise-linear curve `polyline_rz` (shape (N, 2)) -- distance to
    the nearest point on each SEGMENT (clamped to the segment), not
    just the nearest of the N vertices, so the result varies smoothly
    with `point_rz` at the sub-grid-spacing scale. This feeds a
    finite-difference Jacobian (see TargetSurfaceDeviation/
    manifold_opt_twin.py), where a nearest-VERTEX distance's
    grid-cell-boundary jumps would otherwise show up as spurious
    derivative noise.
    """
    a = polyline_rz
    b = np.roll(polyline_rz, -1, axis=0)
    ab = b - a
    ab_len2 = np.sum(ab * ab, axis=1)
    t = np.sum((point_rz[None, :] - a) * ab, axis=1) / np.where(ab_len2 > 0, ab_len2, 1.0)
    t = np.clip(t, 0.0, 1.0)
    closest = a + t[:, None] * ab
    return np.min(np.linalg.norm(point_rz[None, :] - closest, axis=1))


class TargetSurfaceDeviation(Optimizable):
    """
    New (not part of Chris Smiet's original file): the manifold-
    optimization objective of Elder et al. 2024, "Stellarator divertor
    design by optimizing coils for surfaces with sharp corners"
    (arXiv:2510.27624v1), Sec. II.3 eq. 6:

        J_maniopt = sum_i min_S |x_i - S|

    x_i is a seed point's image after tracing forward through exactly
    one field period (SimpleIntegrator.integrate_in_phi -- the same
    plain scipy-ODE tracer SimpleFixedPoint_RZ above uses, NOT
    pyoculus's own CylindricalBfieldSection/Newton machinery: unlike a
    genuine fixed-point search, nothing here needs to already be close
    to a periodic orbit, so a single forward integration per seed point
    is all eq. 6 asks for). S ranges over `target_surfs[i]`'s own cross
    section at that seed point's toroidal angle.

    Unlike SimpleFixedPoint_RZ (which compares the mapped point to ITS
    OWN starting point, for driving a chosen (R, Z) toward becoming an
    actual fixed point), this compares against the nearest point of a
    FIXED target surface -- the boundary the coils were already
    optimized to reproduce (e.g. stage_2_scan_twin.py's
    surf_outboard/surf_inboard) -- so it measures whether the
    divertor-leg region stays locally invariant, not whether a specific
    resonance exists.

    By nfp-periodicity of both the field and `target_surfs[i]`'s own
    geometry, the mapped point's true toroidal angle (seed_phi +
    2*pi/nfp) has the identical target cross section as seed_phi itself
    -- so the target lookup uses seed_phi directly rather than the
    unreduced endpoint angle (equivalent, and avoids relying on
    target_surfs[i].cross_section to correctly unwrap an angle outside
    its own one-field-period domain).
    """

    def __init__(self, field, seed_xyz, target_surfs, nfp, tol=1e-9, ntheta_target=400):
        """
        :param field: simsopt MagneticField (e.g. BiotSavart) to trace through.
        :param seed_xyz: (N, 3) starting positions -- e.g. the crossover-leg
            grid points already used for the flux objective
            (stage_2_scan_twin.py's pos_outboard/pos_inboard, concatenated).
        :param target_surfs: length-N sequence; target_surfs[i] is the
            Surface (e.g. surf_outboard or surf_inboard) seed_xyz[i]'s
            image should stay close to -- a per-point target lets
            outboard- and inboard-seeded points compare against their
            own twin.
        :param nfp: number of field periods (integration advances phi by
            exactly 2*pi/nfp for every seed point).
        :param tol: integrator rtol/atol (see SimpleIntegrator).
        :param ntheta_target: poloidal resolution of the target cross
            sections used for the nearest-point search.
        """
        if len(target_surfs) != len(seed_xyz):
            raise ValueError("target_surfs must have one entry per seed point")
        self.integrator = SimpleIntegrator(field, tol)
        self.nfp = nfp
        self.ntheta_target = ntheta_target
        self.target_surfs = list(target_surfs)
        seed_rphiz = _xyz_to_rphiz(np.asarray(seed_xyz))
        self.seed_RZ = seed_rphiz[:, [0, 2]]
        self.seed_phi = seed_rphiz[:, 1]
        # Target cross sections depend only on surface geometry (fixed
        # throughout the coil optimization), not on the field -- cache
        # them rather than recomputing every residual evaluation.
        self._target_rz_cache = {}
        Optimizable.__init__(self, x0=np.asarray([]), depends_on=[field])

    def _target_polyline_rz(self, i):
        key = (id(self.target_surfs[i]), self.seed_phi[i])
        cached = self._target_rz_cache.get(key)
        if cached is None:
            phi_frac = self.seed_phi[i] / (2 * np.pi)
            xyz = self.target_surfs[i].cross_section(phi_frac, thetas=self.ntheta_target)
            cached = np.stack([np.hypot(xyz[:, 0], xyz[:, 1]), xyz[:, 2]], axis=1)
            self._target_rz_cache[key] = cached
        return cached

    def residuals(self):
        """(N,) array of min|x_i - S| distances -- one LeastSquaresProblem
        residual per seed point (see manifold_opt_twin.py)."""
        out = np.empty(len(self.seed_RZ))
        for i in range(len(self.seed_RZ)):
            phi_end = self.seed_phi[i] + 2 * np.pi / self.nfp
            try:
                mapped_RZ = self.integrator.integrate_in_phi(
                    self.seed_RZ[i], self.seed_phi[i], phi_end
                )
            except Exception as e:
                raise ObjectiveFailure("Failed to integrate field line") from e
            if np.any(np.isnan(mapped_RZ)):
                raise ObjectiveFailure(
                    "Field line left the integration domain (hit event_function)"
                )
            out[i] = _point_to_polyline_min_dist(mapped_RZ, self._target_polyline_rz(i))
        return out

    def J(self):
        """sum of the per-point residuals -- eq. 6 itself, for quick
        diagnostics/printing (the LeastSquaresProblem uses `residuals`,
        i.e. the un-summed per-point array, directly)."""
