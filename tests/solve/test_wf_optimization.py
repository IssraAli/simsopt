...
from simsopt.solve import optimize_wireframe, bnorm_obj_matrices, get_gsco_iteration
...
class WireframeOptimizationTests(unittest.TestCase):
    def test_toroidal_wireframe_bnorm_obj_matrices(self):
        ...
        sq_flux_mat = 0.5 * np.sum((Amat @ wf.currents.reshape((-1, 1)) - cvec) ** 2)
        ...
        Amat2, cvec2 = bnorm_obj_matrices(
            wf, surf_plas, ext_field=mf_tor, verbose=False
        )
        self.assertTrue(np.allclose(Amat, Amat2))
        self.assertFalse(np.allclose(cvec, cvec2))
        sq_flux_mat2 = 0.5 * np.sum((Amat2 @ wf.currents.reshape((-1, 1)) - cvec2) ** 2)
    ...
    def test_toroidal_wireframe_rcls(self):
        ...
        res = optimize_wireframe(wf, "rcls", opt_params, surf_plas, verbose=False)
        ...
        res = optimize_wireframe(wf, "rcls", opt_params, surf_plas, verbose=True)
        ...
        mf_tor = ToroidalField(1.0, -2e-7 * cur_pol)
        res = optimize_wireframe(
            wf, "rcls", opt_params, surf_plas, ext_field=mf_tor, verbose=False
        )
        ...
        self.assertTrue(
            np.isclose(
                cur_pol,
                -enclosed_current(
                    amploop_pol, res["wframe_field"] + mf_tor, n_pts_amploop
                ),
            )
        )
        self.assertTrue(
        ...
        self.assertTrue(
            np.isclose(
                SquaredFlux(surf_plas, res["wframe_field"] + mf_tor).J(), res["f_B"]
            )
        )
        self.assertTrue(np.isclose(res["f"], res["f_B"] + res["f_R"]))
        self.assertTrue(
            np.isclose(
                res["f_R"], 0.5 * opt_params["reg_W"] ** 2 * np.sum(res["x"] ** 2)
            )
        )

        # Verify that same solution is obtained when the bnormal and objective
        # matrices are supplied by the user
        res2 = optimize_wireframe(
            wf, "rcls", opt_params, Amat=res["Amat"], bvec=res["bvec"], verbose=True
        )
    ...
    def test_toroidal_wireframe_gsco(self):
        ...
        with self.assertRaises(ValueError):
            optimize_wireframe(wf, "gsco", dict(), surf_plas=surf_plas, verbose=False)
        params_no_lambda = dict(std_params)
        del params_no_lambda["lambda_S"]
        with self.assertRaises(ValueError):
            optimize_wireframe(
                wf, "gsco", params_no_lambda, surf_plas=surf_plas, verbose=False
            )

        # Basic optimization in an external toroidal field
        res0 = optimize_wireframe(
            wf, "gsco", std_params, surf_plas=surf_plas, ext_field=mf_tor, verbose=False
        )
        ...
        self.assertTrue(np.max(np.abs(wf.currents)) <= std_params["max_current"])
        self.assertTrue(wf.check_constraints())
        ...
        self.assertTrue(
            np.isclose(
                SquaredFlux(surf_plas, res0["wframe_field"] + mf_tor).J(), res0["f_B"]
            )
        )
        self.assertTrue(
            np.isclose(res0["f"], res0["f_B"] + std_params["lambda_S"] * res0["f_S"])
        )
        self.assertTrue(
            np.isclose(
                res0["f_S"], 0.5 * np.sum(np.abs(res0["x"]) > wf.constraint_atol)
            )
        )
        ...
        res5 = optimize_wireframe(
            wf,
            "gsco",
            params_no_xing_x_init,
            surf_plas=surf_plas,
            ext_field=mf_tor,
            verbose=False,
        )
        for i in res5["iter_hist"]:
            self.assertTrue(
                np.allclose(
                    get_gsco_iteration(i, res4, wf), get_gsco_iteration(i, res5, wf)
                )
            )
        ...
        res6 = optimize_wireframe(
            wf2,
            "gsco",
            params_no_xing_1_curr,
            surf_plas=surf_plas,
            ext_field=mf_tor,
            verbose=False,
        )
        ...
        res7 = optimize_wireframe(
            wf2,
            "gsco",
            params_no_xing_1_curr_1_loop,
            surf_plas=surf_plas,
            ext_field=mf_tor,
            verbose=False,
        )
        self.assertEqual(
            np.max(np.abs(res7["loop_count"])),
    ...
    def test_optimize_wireframe_errors_and_bnorm_target(self):
        ...
        with self.assertRaisesRegex(
            ValueError, "Input `wframe` must be a ToroidalWireframe class instance"
        ):
        ...
        with self.assertRaisesRegex(
            ValueError,
            "If `Amat` and `bvec` are provided, the following parameters must not be provided",
        ):
            optimize_wireframe(
                wf,
                "rcls",
                ...
                bvec=dummy_vector,
                ext_field=True,
            )  # Using ext_field to trigger this error

        # Test 4: Either surf_plas or both Amat and bvec must be supplied
        with self.assertRaisesRegex(
            ValueError, "`surf_plas` or `Amat` and `bvec` must be supplied"
        ):
        ...
        with self.assertRaisesRegex(
            ValueError, "Input `Amat` has inconsistent dimensions"
        ):
            optimize_wireframe(
                wf, "rcls", {"reg_W": 1e-10}, Amat=wrong_matrix, bvec=np.ones((5, 1))
            )
        ...
        with self.assertRaisesRegex(
            ValueError, "Input `bnorm_target` must have the same"
        ):
            optimize_wireframe(
                wf,
                "rcls",
        ...
        amploop_tor.set("xc(1)", 2 * surf_wf.get_rc(1, 0))
        amploop_tor.set("zs(1)", 2 * surf_wf.get_zs(1, 0))
        res_baseline = optimize_wireframe(
            wf,
            "rcls",
            opt_params,
            surf_plas=surf_plas,
        ...
        res_with_target = optimize_wireframe(
            wf,
            "rcls",
            opt_params,
            surf_plas=surf_plas,
        ...
        self.assertFalse(
            np.allclose(res_baseline["bvec"], res_with_target["bvec"]),
        ...
        res_with_target2 = optimize_wireframe(
            wf,
            "rcls",
            opt_params,
            surf_plas=surf_plas,
        ...
        print(
            "Amat @ x - bvec",
            res_with_target["Amat"] @ res_with_target["x"] - res_with_target["bvec"],
            res_with_target2["Amat"] @ res_with_target2["x"] - res_with_target2["bvec"],
        )
        self.assertFalse(
            np.allclose(res_with_target["x"], res_with_target2["x"]),
