...
@unittest.skipIf(
    (virtual_casing is None) or (MPI is None) or (vmec_mod is None),
...
class VirtualCasingTests(unittest.TestCase):
    ...
    def subtest_bnorm_benchmark(self, use_stellsym):
        ...
        curpol = (2 * np.pi / nfp) * (
            1.5 * vmec.wout.bsubvmnc[0, -1] - 0.5 * vmec.wout.bsubvmnc[0, -2]
        )
        ...
        logger.info(
            "Diff between BNORM and virtual_casing: "
            f"abs={np.max(np.abs(difference))}, rel={np.max(np.abs(rel_difference))}"
        )
        np.testing.assert_allclose(
            B_external_normal_bnorm, vc.B_external_normal, atol=0.0061
        )
    ...
    def test_save_load(self):
        ...
        with ScratchDir("."):
            ...
            for variable in variables:
                ...
                logger.info(
                    f"Variable {variable} in vc1 is {variable1} and in vc2 is {variable2}"
                )
    ...
    def test_vacuum(self):
        ...
        with ScratchDir("."):
            ...
            logger.info(
                f"ntheta: {vc.src_ntheta}  B_external.shape: {vc.B_external.shape}"
            )
            logger.info(
                f"max(|B_external - B_total|): {np.max(np.abs(vc.B_external - vc.B_total))}"
            )
            logger.info(
                f"max(|B_external_normal|): {np.max(np.abs(vc.B_external_normal))}"
            )
    ...
    def test_stellsym(self):
        ...
        src_ntheta = 12
        vc = VirtualCasing.from_vmec(
            vmec, src_nphi=src_nphi, src_ntheta=src_ntheta, use_stellsym=False
        )
        Bn_flipped = -np.rot90(np.rot90(vc.B_external_normal))
        Bn_flipped = np.roll(np.roll(Bn_flipped, 1, axis=0), 1, axis=1)
        logger.info(
            f"max diff in B_external_normal: {np.max(np.abs(vc.B_external_normal - Bn_flipped))}"
        )
        np.testing.assert_allclose(vc.B_external_normal, Bn_flipped, atol=1e-10)
        vc_ss = VirtualCasing.from_vmec(
            vmec, src_nphi=src_nphi // 4, src_ntheta=src_ntheta, use_stellsym=True
        )
        ...
        logger.info(
            f"max diff in B_external_normal: {np.max(np.abs(vc.B_external_normal[idxs, :] - vc_ss.B_external_normal))}"
        )
        np.testing.assert_allclose(
            vc.B_external_normal[idxs, :], vc_ss.B_external_normal, atol=1e-5
        )
        """
        import matplotlib.pyplot as plt
        plt.subplot(1, 2, 1)
