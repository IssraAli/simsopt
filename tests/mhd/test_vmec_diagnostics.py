...
class QuasisymmetryRatioResidualWoutTests(unittest.TestCase):
        ...
        for surfaces, weights in zip(ss, weightss):
            ...
            for m, n in zip(ms, ns):
                ...
                np.testing.assert_allclose(
                    qs_ref.total(), qs.total(), atol=0, rtol=1e-2
                )
    ...
    def test_profile(self):
        ...
        m = 1
        n = 1
        qs_profile = QuasisymmetryRatioResidual(
            vmec, surfs, helicity_m=m, helicity_n=n, weights=weights
        )
        profile = qs_profile.profile()
        np.testing.assert_allclose(np.sum(profile), qs_profile.total())
        for j in range(len(surfs)):
            qs_single = QuasisymmetryRatioResidual(
                vmec, [surfs[j]], helicity_m=m, helicity_n=n, weights=[weights[j]]
            )
    ...
    def test_compute(self):
        ...
        r = qs.compute()
        np.testing.assert_allclose(
            r.bsupu * r.d_B_d_theta + r.bsupv * r.d_B_d_phi, r.B_dot_grad_B
        )
        np.testing.assert_allclose(
            r.B_cross_grad_B_dot_grad_psi,
...
class QuasisymmetryRatioResidualTests(unittest.TestCase):
    def test_axisymmetry(self):
        ...
        with ScratchDir("."):
            ...
            for surfaces in [[0.5], [0.3, 0.6]]:
                qa = QuasisymmetryRatioResidual(
                    vmec, surfaces, helicity_m=1, helicity_n=0
                )
                residuals = qa.residuals()
                logger.info(f"max QA error: {np.max(np.abs(residuals))}")
                np.testing.assert_allclose(
                    residuals, np.zeros_like(residuals), atol=2e-6
                )

                qh = QuasisymmetryRatioResidual(
                    vmec, surfaces, helicity_m=1, helicity_n=1
                )
                logger.info(f"QH error: {qh.total()}")
                self.assertTrue(qh.total() > 0.01)

                qp = QuasisymmetryRatioResidual(
                    vmec, surfaces, helicity_m=0, helicity_n=1
                )
    ...
    def test_good_qa(self):
        ...
        with ScratchDir("."):
            vmec = Vmec(
                os.path.join(
                    TEST_DIR, "input.simsopt_nfp2_QA_20210328-01-020_000_000251"
                )
            )
            qa = QuasisymmetryRatioResidual(vmec, 0.5, helicity_m=1, helicity_n=0)
            total = qa.total()
            logger.info(
                f"QA error for simsopt_nfp2_QA_20210328-01-020_000_000251: {total}"
            )
    ...
    def test_good_qh(self):
        ...
        with ScratchDir("."):
            vmec = Vmec(
                os.path.join(TEST_DIR, "input.20210406-01-002-nfp4_QH_000_000240")
            )

            qh = QuasisymmetryRatioResidual(vmec, 0.5, helicity_m=1, helicity_n=-1)
            logger.info(
                f"QH error (n=-1) for 20210406-01-002-nfp4_QH_000_000240: {qh.total()}"
            )
            self.assertTrue(qh.total() < 5e-5)

            qh2 = QuasisymmetryRatioResidual(vmec, 0.5, helicity_m=1, helicity_n=1)
            logger.info(
                f"QH error (n=+1) for 20210406-01-002-nfp4_QH_000_000240: {qh2.total()}"
            )
    ...
    def test_independent_of_scaling(self):
        ...
        with ScratchDir("."):
            ...
            qs1 = QuasisymmetryRatioResidual(
                vmec, [0, 0.7, 1], helicity_m=1, helicity_n=-1, weights=[0.8, 1.1, 0.9]
            )
            ...
            qs2 = QuasisymmetryRatioResidual(
                vmec, [0, 0.7, 1], helicity_m=1, helicity_n=-1, weights=[0.8, 1.1, 0.9]
            )
            ...
            logger.info(
                "Max difference in residuals after scaling vmec:"
                + str(np.max(np.abs(residuals1 - residuals2)))
            )
            np.testing.assert_allclose(residuals1, residuals2, rtol=1e-10, atol=1e-10)
            np.testing.assert_allclose(
                results1.total, results2.total, rtol=1e-10, atol=1e-10
            )
            np.testing.assert_allclose(
                results1.profile, results2.profile, rtol=1e-10, atol=1e-10
            )
    ...
    def test_iota_0(self):
        ...
        with ScratchDir("."):
            ...
            logger.info(
                f"QuasisymmetryRatioResidual for a vacuum axisymmetric config with iota = 0: {qs.total()}"
            )
            self.assertTrue(qs.total() < 1e-12)


@unittest.skipIf(
    (MPI is None) or (vmec is None), "Valid Python interface to VMEC not found"
)
            ...
            for grid_option in range(3):
                ...
                with ScratchDir("."):
                    ...
                    if grid_option == 0:
                        ...
                        ntheta = 55
                        nphi = 52
                        Bx, By, Bz = B_cartesian(
                            vmec, ntheta=ntheta, nphi=nphi, range="field period"
                        )
                        # The previous line runs vmec so now vmec.wout.nfp is available.
                        theta1D = np.linspace(0, 2 * np.pi, ntheta, endpoint=False)
                        phi1D = np.linspace(
                            0, 2 * np.pi / vmec.wout.nfp, nphi, endpoint=False
                        )
                    elif grid_option == 2:
                    ...
                    angle = (
                        vmec.wout.xm_nyq[:, None, None] * theta[None, :, :]
...
class IotaTargetMetricTests(unittest.TestCase):
    def test_IotaTargetMetric_J(self):
        ...
        with ScratchDir("."):
            ...
            metric2 = (
                0.5
                * np.sum(
                    (vmec.wout.iotaf - target_function(vmec.s_full_grid)) ** 2 * stencil
                )
                * vmec.ds
            )
    ...
    def test_IotaTargetMetric_dJ(self):
        ...
        with ScratchDir("."):
            ...
            epsilon = 1.0e-4  # FD step size
            adjoint_epsilon = 1.0e-2  # perturbation amplitude for adjoint solve
            ...
            self.assertLessEqual(relative_error, 5.0e-2)
...
class IotaWeightedTests(unittest.TestCase):
    def test_IotaWeighted_J(self):
        ...
        with ScratchDir("."):
            ...
            def weight_function(s):
                return np.exp(-((s - s_center) ** 2) / 0.01**2)
    ...
    def test_IotaWeighted_dJ(self):
        ...
        epsilon = 1.0e-4  # FD step size
        adjoint_epsilon = 5.0e-2  # perturbation amplitude for adjoint solve
...
class WellWeightedTests(unittest.TestCase):
    def test_WellWeighted_J(self):
        ...
        with ScratchDir("."):
            ...
            def weight1(s):
                return np.exp(-(s**2) / 0.01**2)

            def weight2(s):
                return np.exp(-((1 - s) ** 2) / 0.01**2)
    ...
    def test_WellWeighted_dJ(self):
        ...
        random direction.
        """
        epsilon = 1.0e-2  # FD step size
        adjoint_epsilon = 1.0e0  # perturbation amplitude for adjoint solve

        def weight1(s):
            return np.exp(-(s**2) / 0.5**2)

        def weight2(s):
            return np.exp(-((1 - s) ** 2) / 0.5**2)
...
class VmecComputeGeometryTests(unittest.TestCase):
    def test_1d_matches_3d(self):
        ...
        np.testing.assert_allclose(
            results1.grad_psi_dot_grad_psi, results2.grad_psi_dot_grad_psi
        )
        np.testing.assert_allclose(
            results1.B_cross_kappa_dot_grad_psi, results2.B_cross_kappa_dot_grad_psi
        )
    ...
    def test_compare_to_desc(self):
        ...
        desc_data[0, :, :] = np.array(
            [
                [0.0232505427, 0.0136928264, 0.0045250425, 0.0045250425, 0.0136928264],
        ...
        desc_data[1, :, :] = np.array(
            [
                [0.0682776505, 0.0419440941, 0.0159952307, 0.0159952307, 0.0419440941],
        ...
        np.testing.assert_allclose(
            simsopt_data.grad_psi_dot_grad_psi, desc_data, rtol=0.005, atol=0.0005
        )
    ...
    def test_derivatives(self):
        ...
        def d2_d_theta2(f):
            return (np.roll(f, -1, axis=1) - 2 * f + np.roll(f, 1, axis=1)) / (
                dtheta**2
            )
        ...
        np.testing.assert_allclose(
            data.d2_R_d_theta_vmec_d_phi, d_d_theta(d_d_phi(data.R)), atol=2e-3
        )
        np.testing.assert_allclose(
            data.d2_Z_d_theta_vmec_d_phi, d_d_theta(d_d_phi(data.Z)), atol=2e-3
        )
        np.testing.assert_allclose(
            data.d2_R_d_theta_vmec2, d2_d_theta2(data.R), atol=4e-4
        )
        np.testing.assert_allclose(
            data.d2_Z_d_theta_vmec2, d2_d_theta2(data.Z), atol=3e-4
        )
        ...
        np.testing.assert_allclose(
            s_mean(data.d2_R_d_s_d_phi), d_d_s(data.d_R_d_phi), atol=3e-6
        )
        np.testing.assert_allclose(
            s_mean(data.d2_Z_d_s_d_phi), d_d_s(data.d_Z_d_phi), atol=2e-6
        )
        np.testing.assert_allclose(
            s_mean(data.d2_R_d_s_d_theta_vmec), d_d_s(data.d_R_d_theta_vmec), atol=2e-6
        )
        np.testing.assert_allclose(
            s_mean(data.d2_Z_d_s_d_theta_vmec), d_d_s(data.d_Z_d_theta_vmec), atol=2e-6
        )

        np.testing.assert_allclose(
            data.d_B_sup_theta_vmec_d_theta_vmec,
            d_d_theta(data.B_sup_theta_vmec),
            atol=2e-2,
        )
        np.testing.assert_allclose(
            data.d_B_sup_theta_vmec_d_phi, d_d_phi(data.B_sup_theta_vmec), atol=4e-3
        )
        np.testing.assert_allclose(
            data.d_B_sup_phi_d_theta_vmec, d_d_theta(data.B_sup_phi), atol=8e-3
        )
        np.testing.assert_allclose(
            data.d_B_sup_phi_d_phi, d_d_phi(data.B_sup_phi), atol=8e-4
        )
        np.testing.assert_allclose(
            s_mean(data.d_B_sup_phi_d_s), d_d_s(data.B_sup_phi), atol=6e-6
        )
        np.testing.assert_allclose(
            s_mean(data.d_B_sup_theta_vmec_d_s), d_d_s(data.B_sup_theta_vmec), atol=4e-5
        )
    ...
    def test_basic_non_stellsym(self):
        ...
        R_precalc = np.array(
            [
                [
                    [6.85, 4.571410161513775, 3.8785898384862243, 6.85],
                    [4.5, 3.721410161513776, 3.0285898384862238, 4.5],
                    [5.65, 4.005384757729337, 5.044615242270662, 5.65],
                    [8.0, 4.855384757729337, 5.894615242270662, 8.0],
                    [6.85, 4.571410161513775, 3.8785898384862247, 6.85],
                ]
            ]
        )
        d2_Z_d_theta_vmec_d_phi_precalc = np.array(
            [
                [
                    [
                        -0.5,
        ...
        d_B_d_theta_vmec_precalc = np.array(
            [
                [
                    [
                        3.797382850490039,
        ...
        np.testing.assert_allclose(
            data.d2_Z_d_theta_vmec_d_phi, d2_Z_d_theta_vmec_d_phi_precalc, rtol=1e-5
        )
        np.testing.assert_allclose(
            data.d_B_d_theta_vmec, d_B_d_theta_vmec_precalc, rtol=1e-5
        )
        # Non-regression test, comparing the two ways to calculate sqrt(g)
        np.testing.assert_allclose(
            data.sqrt_g_vmec_alt, data.sqrt_g_vmec, rtol=1e-4, atol=1e-4
        )
...
class VmecFieldlinesTests(unittest.TestCase):
    def test_fieldline_grids(self):
        ...
        for js in range(fl.ns):
            for jalpha in range(fl.nalpha):
                np.testing.assert_allclose(
                    fl.theta_pest[js, jalpha, :], theta, atol=1e-15
                )
                np.testing.assert_allclose(
                    fl.theta_pest[js, jalpha, :] - fl.iota[js] * fl.phi[js, jalpha, :],
                    alpha[jalpha],
                    atol=1e-15,
                )
    ...
    def test_consistency(self):
        ...
        for filename in filenames:
            for j_theta_phi_in in range(2):
                logger.info(
                    f"Testing vmec_fieldline for file {filename}, j_theta_phi_in={j_theta_phi_in}"
                )
                ...
                np.testing.assert_allclose(
                    fl.sqrt_g_vmec, fl.sqrt_g_vmec_alt, rtol=1e-4, atol=1e-4
                )

                # Verify that (B dot grad theta_pest) / (B dot grad phi) = iota
                should_be_iota = (
                    fl.B_sup_theta_vmec * (1 + fl.d_lambda_d_theta_vmec)
                    + fl.B_sup_phi * fl.d_lambda_d_phi
                ) / fl.B_sup_phi
                for js in range(ns):
                    np.testing.assert_allclose(
                        fl.iota[js], should_be_iota[js, :, :], rtol=1e-4, atol=1e-4
                    )
                ...
                # from VMEC.
                test_arr = (
                    fl.B_X * fl.d_X_d_theta_vmec
                    + fl.B_Y * fl.d_Y_d_theta_vmec
                    + fl.B_Z * fl.d_Z_d_theta_vmec
                )
                np.testing.assert_allclose(
                    test_arr, fl.B_sub_theta_vmec, rtol=0.01, atol=0.01
                )

                test_arr = (
                    fl.B_X * fl.d_X_d_s + fl.B_Y * fl.d_Y_d_s + fl.B_Z * fl.d_Z_d_s
                )
                ...
                test_arr = (
                    fl.B_X * fl.grad_s_X + fl.B_Y * fl.grad_s_Y + fl.B_Z * fl.grad_s_Z
                )
                np.testing.assert_allclose(test_arr, 0, atol=1e-14)

                test_arr = (
                    fl.B_X * fl.grad_phi_X
                    + fl.B_Y * fl.grad_phi_Y
                    + fl.B_Z * fl.grad_phi_Z
                )
                np.testing.assert_allclose(test_arr, fl.B_sup_phi, rtol=1e-4)

                test_arr = (
                    fl.B_X * fl.grad_theta_vmec_X
                    + fl.B_Y * fl.grad_theta_vmec_Y
                    + fl.B_Z * fl.grad_theta_vmec_Z
                )
                np.testing.assert_allclose(test_arr, fl.B_sup_theta_vmec, rtol=2e-4)

                # Check 2 ways of computing B_cross_grad_s_dot_grad_alpha:
                np.testing.assert_allclose(
                    fl.B_cross_grad_s_dot_grad_alpha,
                    fl.B_cross_grad_s_dot_grad_alpha_alternate,
                    rtol=1e-3,
                )
                ...
                np.testing.assert_allclose(
                    ...
                    fl.B_cross_grad_B_dot_grad_alpha_alternate,
                    atol=0.02,
                )
    ...
    def test_stella_regression(self):
        ...
        vmec = Vmec(
            os.path.join(
                TEST_DIR, "wout_LandremanPaul2021_QA_reactorScale_lowres_reference.nc"
            )
        )
        s = [0.5, 1.0]
        alpha = np.linspace(0, 2 * np.pi, 3, endpoint=False)
        ...
        np.testing.assert_allclose(
            fl.theta_vmec[1, :, :], theta_vmec_reference, rtol=3e-5, atol=3e-5
        )
        ...
        np.testing.assert_allclose(
            fl.theta_vmec[0, :, :], theta_vmec_reference, rtol=3e-10, atol=3e-10
        )

        modB_reference = np.array(
            [
                [
                    5.5314557915824105,
                    5.4914726973937169,
        ...
        np.testing.assert_allclose(
            fl.modB[0, :, :], modB_reference, rtol=1e-10, atol=1e-10
        )

        gradpar_reference = np.array(
            [
                [
                    0.16966605523903308,
                    0.16128460473382750,
        ...
        np.testing.assert_allclose(
            L_reference * fl.B_sup_phi[0, :, :] / fl.modB[0, :, :],
            gradpar_reference,
            rtol=1e-11,
            atol=1e-11,
        )
        ...
        vmec = Vmec(
            os.path.join(
                TEST_DIR, "wout_W7-X_without_coil_ripple_beta0p05_d23p4_tm_reference.nc"
            )
        )
        s = [0.5]
        nalpha = 3
        ...
        with open(
            os.path.join(
                TEST_DIR, "geometry_W7-X_without_coil_ripple_beta0p05_d23p4_tm.dat"
            )
        ) as f:
            lines = f.readlines()
        ...
        for j in range(nalpha):
            ...
            np.testing.assert_allclose(
                fl.bmag[0, j, :], np.fromstring(lines[8 + j], sep=" "), rtol=1e-6
            )
            np.testing.assert_allclose(
                fl.gradpar_phi[0, j, :],
                np.fromstring(lines[12 + j], sep=" "),
                rtol=1e-6,
            )
            np.testing.assert_allclose(
                fl.gds2[0, j, :], np.fromstring(lines[16 + j], sep=" "), rtol=2e-4
            )
            np.testing.assert_allclose(
                fl.gds21[0, j, :], np.fromstring(lines[20 + j], sep=" "), atol=2e-4
            )
            np.testing.assert_allclose(
                fl.gds22[0, j, :], np.fromstring(lines[24 + j], sep=" "), atol=2e-4
            )
            np.testing.assert_allclose(
                -1 * fl.gbdrift[0, j, :],
                np.fromstring(lines[28 + j], sep=" "),
                atol=2e-4,
            )
            np.testing.assert_allclose(
                fl.gbdrift0[0, j, :], np.fromstring(lines[32 + j], sep=" "), atol=1e-4
            )
            np.testing.assert_allclose(
                -1 * fl.cvdrift[0, j, :],
                np.fromstring(lines[36 + j], sep=" "),
                atol=2e-4,
            )
            np.testing.assert_allclose(
                fl.cvdrift0[0, j, :], np.fromstring(lines[40 + j], sep=" "), atol=1e-4
            )
    ...
    def test_axisymm(self):
        ...
        """
        vmec = Vmec(
            os.path.join(TEST_DIR, "wout_circular_tokamak_aspect_100_reference.nc")
        )
        ...
        np.testing.assert_allclose(
            fl.modB.flatten(), B0 * (1 - eps * np.cos(theta)), rtol=0.0002
        )
        np.testing.assert_allclose(
            fl.B_sup_theta_vmec.flatten(), B0 / (safety_factor_q * R), rtol=0.0006
        )
        np.testing.assert_allclose(
            fl.B_cross_grad_B_dot_grad_psi.flatten(),
            -(B0**3) * eps * np.sin(theta),
            rtol=0.03,
        )
        np.testing.assert_allclose(
            fl.B_cross_kappa_dot_grad_psi.flatten(),
            -(B0**2) * eps * np.sin(theta),
            rtol=0.02,
        )
        np.testing.assert_allclose(
            fl.grad_psi_dot_grad_psi.flatten(), B0 * B0 * Aminor * Aminor, rtol=0.03
        )
        np.testing.assert_allclose(
            fl.grad_alpha_dot_grad_psi.flatten(),
            -fl.toroidal_flux_sign * phi * d_iota_d_r * Aminor * B0,
            rtol=0.02,
        )
        np.testing.assert_allclose(
            fl.grad_alpha_dot_grad_alpha.flatten(),
            1 / (Aminor * Aminor) + (phi * phi * d_iota_d_r * d_iota_d_r),
            rtol=0.04,
        )
        np.testing.assert_allclose(
            fl.B_cross_grad_B_dot_grad_alpha.flatten(),
            fl.toroidal_flux_sign
            * (B0 * B0 / Aminor)
            * (-np.cos(theta) / R + phi * d_iota_d_r * eps * np.sin(theta)),
            atol=0.006,
        )
        np.testing.assert_allclose(
            fl.B_cross_kappa_dot_grad_alpha.flatten(),
            fl.toroidal_flux_sign
            * (B0 / Aminor)
            * (-np.cos(theta) / R + phi * d_iota_d_r * eps * np.sin(theta)),
            atol=0.006,
    ...
    def test_plot(self):
        ...
        vmec_fieldlines(
            vmec, s=[0.25, 0.5], alpha=alpha, phi1d=phi, plot=True, show=False
        )
