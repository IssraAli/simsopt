...
from simsopt.mhd.bootstrap import (
...
class BootstrapTests(unittest.TestCase):
    def test_compute_trapped_fraction(self):
        ...
        for jdim in [2, 3]:
            ...
            np.testing.assert_allclose(
                fsa_b2, [13.0**2 + 0.5 * 2.6**2, 9.0**2 + 0.5 * 3.7**2]
            )
            np.testing.assert_allclose(
                fsa_1overB,
    ...
    def test_trapped_fraction_Kim(self):
        ...
        for jdim in [2, 3]:
            ...
            Bmin, Bmax, epsilon_out, fsa_B2, fsa_1overB, f_t = compute_trapped_fraction(
                modB, sqrtg
            )

            f_t_Kim = (
                1.46 * np.sqrt(epsilon_in) - 0.46 * epsilon_in
            )  # Eq (C18) in Kim et al
            ...
            # Eq (A8):
            np.testing.assert_allclose(
                fsa_B2, B0 * B0 / np.sqrt(1 - epsilon_in**2), rtol=1e-6
            )
            np.testing.assert_allclose(
                f_t, f_t_Kim, rtol=0.1, atol=0.07
            )  # We do not expect precise agreement.
    ...
    def test_Redl_second_pass(self):
        ...
        d_p_d_s = (
            ne_s * d_Te_d_s_J + Te_J * d_ne_d_s + ni_s * d_Ti_d_s_J + Ti_J * d_ni_d_s
        )
        ...
        dTeds_term = factors * (
            L31 * p / pe * (ne_s * d_Te_d_s_J) / p + L32 * (d_Te_d_s_J / Te_J)
        )
        dTids_term = factors * (
            L31 * p / pe * (ni_s * d_Ti_d_s_J) / p
    ...
    def test_compare_geometry_methods_tokamak(self):
        ...
        logging.info(
            f"Bmin:  vmec={geom1.Bmin}  booz={geom2.Bmin}  diff={geom1.Bmin - geom2.Bmin}"
        )
        logging.info(
            f"Bmax:  vmec={geom1.Bmax}  booz={geom2.Bmax}  diff={geom1.Bmax - geom2.Bmax}"
        )
        logging.info(
            f"epsilon:  vmec={geom1.epsilon}  booz={geom2.epsilon}  diff={geom1.epsilon - geom2.epsilon}"
        )
        logging.info(
            f"fsa_B2:  vmec={geom1.fsa_B2}  booz={geom2.fsa_B2}  diff={geom1.fsa_B2 - geom2.fsa_B2}"
        )
        logging.info(
            f"fsa_1overB:  vmec={geom1.fsa_1overB}  booz={geom2.fsa_1overB}  diff={geom1.fsa_1overB - geom2.fsa_1overB}"
        )
        logging.info(
            f"f_t:  vmec={geom1.f_t}  booz={geom2.f_t}  diff={geom1.f_t - geom2.f_t}"
        )
    ...
    def test_Redl_sfincs_tokamak_benchmark(self):
        ...
        for geom_method in range(2):
            if geom_method == 0:
                ...
                jdotB, details = j_dot_B_Redl(
                    ne, Te, Ti, Zeff, helicity_n, geom=geom, plot=False
                )
            else:
    ...
    def test_VmecRedlBootstrapMismatch_1(self):
        ...
        obj1J = obj1.J()
        logging.info(
            f"test_VmecRedlBootstrapMismatch_1: objective is {obj1J:.3e} "
        ...
        obj2J = obj2.J()
        logging.info(
            f"test_VmecRedlBootstrapMismatch_1: objective is {obj2J:.3e} "
    ...
    def test_VmecRedlBootstrapMismatch_independent_of_ns(self):
        ...
        with ScratchDir("."):
            ...
            obj1 = VmecRedlBootstrapMismatch(
                geom1,
                ne,
                Te,
                Ti,
            ...
            logging.info(
                f"resolution1: {obj1J:.4e}  resolution2: {obj2J:.4e}  rel diff: {rel_diff}"
            )
