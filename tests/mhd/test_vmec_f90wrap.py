...
run_modes = {
    ...
    "output": 8,
    "main": 45,
}  # STELLOPT uses 61; V3FIT uses 45
...
@unittest.skip(
    "These tests are obsolete due to the refactoring of vmec.py and vmec_core.py"
)
class F90wrapVmecTests(unittest.TestCase):
    ...
    def tearDown(self):
        ...
        vmec.runvmec(self.ictrl, self.filename, self.verbose, self.fcomm, reset_file)
    ...
    def test_run_read(self):
        ...
        self.assertAlmostEqual(vmec.read_wout_mod.betatot, 0.0426211525919469, places=4)

        print("iotaf.shape:", vmec.read_wout_mod.iotaf.shape)
        print("rmnc.shape:", vmec.read_wout_mod.rmnc.shape)

        self.assertAlmostEqual(
            vmec.read_wout_mod.iotaf[-1], 0.6556508142482989, places=4
        )

        self.assertAlmostEqual(
            vmec.read_wout_mod.rmnc[0, 0], 1.4760749266902973, places=4
        )
