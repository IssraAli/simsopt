...
class FiniteDifferenceStepsTests(unittest.TestCase):
    def test_abs_step(self):
        ...
        np.testing.assert_allclose(
            finite_difference_steps(x, abs_step=abs_step, rel_step=0),
            np.full(N, abs_step),
        )

        x = np.array(
            [0.22703548, -0.09694371, 0.36863702, -0.46309711, 0.164472, -0.08652083]
        )
        np.testing.assert_allclose(
            finite_difference_steps(x, abs_step=abs_step, rel_step=0),
            np.full(6, abs_step),
    ...
    def test_rel_step(self):
        ...
        rel_step = 1.0e-5
        np.testing.assert_allclose(
            finite_difference_steps(x, abs_step=0, rel_step=rel_step),
            np.abs(x) * rel_step,
        )
    ...
    def test_mixed(self):
        ...
        x0 = np.array([0.08, 0.092130271, 0.109012617, 0.08, 0.08, 0.08, 0.151686311])

        np.testing.assert_allclose(
            finite_difference_steps(x, abs_step=0.08, rel_step=0.1), x0
        )

        x = np.array([0, 1.21923073, 0.85723015, -0.08222701, 0, -0.9645991, 0, 0])
        ...
        np.testing.assert_allclose(
            finite_difference_steps(x, abs_step=1.0e-7, rel_step=1.0e-3), x0
        )
    ...
    def test_zero_step(self):
        ...
        """
        x = np.array([0.22703548, -0.09694371, 0.36863702, -0.46309711, 0, -0.08652083])
...
class NestedListsToArrayTests(unittest.TestCase):
    def test_nested_lists_to_array(self):
        ...
        arr1 = nested_lists_to_array(list_of_lists)
        arr2 = np.array([[42, 0, 0], [1, 2, 3]])
        np.testing.assert_allclose(arr1, arr2)

        list_of_lists = [[None, 42], [1, 2, 3]]
        arr1 = nested_lists_to_array(list_of_lists)
        arr2 = np.array([[0, 42, 0], [1, 2, 3]])
        np.testing.assert_allclose(arr1, arr2)

        list_of_lists = [[42, 43, 44], [1, 2, 3]]
        arr1 = nested_lists_to_array(list_of_lists)
        arr2 = np.array([[42, 43, 44], [1, 2, 3]])
        np.testing.assert_allclose(arr1, arr2)

        list_of_lists = [[42, 43, 44, 45], [1, 2, 3]]
        arr1 = nested_lists_to_array(list_of_lists)
        arr2 = np.array([[42, 43, 44, 45], [1, 2, 3, 0]])
        np.testing.assert_allclose(arr1, arr2)
