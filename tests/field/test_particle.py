...
from simsopt.field.tracing import (
    ...
    compute_gc_radius,
    gc_to_fullorbit_initial_guesses,
...
class ParticleTracingTesting(unittest.TestCase):
    def __init__(self, *args, **kwargs):
        ...
        bs.set_points(np.asarray([[0.0, 0.0, 0.0]])).GradAbsB()
        self.bsh = bsh
        self.ma = ma
    ...
    def test_gc_to_full(self):
        ...
        xyz, vxyz, _ = gc_to_fullorbit_initial_guesses(
            bsh, xyz_gc, vtangs, vtotal, m, q, eta=etas[0]
        )
        for eta in etas[1:]:
            tmp = gc_to_fullorbit_initial_guesses(
                bsh, xyz_gc, vtangs, vtotal, m, q, eta=eta
            )
            xyz += tmp[0]
            vxyz += tmp[1]
            radius = tmp[2]
            assert np.allclose(
                np.abs(np.linalg.norm(xyz_gc - tmp[0], axis=1) - radius), 0
            )
            assert np.all(
                np.abs(np.linalg.norm(tmp[1], axis=1) - vtotal) / vtotal < 1e-10
            )
        xyz *= 1 / N
        vxyz *= 1 / N
    ...
    def test_energy_conservation(self):
        ...
        gc_tys, gc_phi_hits = trace_particles_starting_on_curve(
            ...
            Ekin=Ekin,
            umin=-0.5,
            umax=-0.25,  # pitch angle so that we have both par and perp contribution
            phis=[],
            mode="gc_vac",
            tol=1e-11,
        )
        ...
        for i in range(nparticles):
            ...
            muInitial = Ekin / (m * AbsBs_gc[0, 0]) - vParInitial**2 / (
                2 * AbsBs_gc[0, 0]
            )
            for j in range(N):
                v_fo = fo_ty[idxs[j], 4:]
                ...
                energy_fo = np.append(
                    energy_fo,
                    (m / 2) * (v_fo[[0]] ** 2 + v_fo[[1]] ** 2 + v_fo[[2]] ** 2),
                )
                energy_gc = np.append(
                    energy_gc, (m / 2) * (v_gc**2 + 2 * muInitial * AbsBs_gc[j, 0])
                )
                mu_fo = np.append(mu_fo, vperp**2 / (2 * AbsBs[j, 0]))
                mu_gc = np.append(
                    mu_gc, Ekin / (m * AbsBs_gc[j, 0]) - v_gc**2 / (2 * AbsBs_gc[j, 0])
                )
            energy_fo_error = np.log10(
                np.abs(energy_fo - energy_fo[0] + 1e-30) / energy_fo[0]
            )
            energy_gc_error = np.log10(
                np.abs(energy_gc - energy_gc[0] + 1e-30) / energy_gc[0]
            )
            mu_fo_error = np.log10(np.abs(mu_fo - mu_fo[0] + 1e-30) / mu_fo[0])
            mu_gc_error = np.log10(np.abs(mu_gc - mu_gc[0] + 1e-30) / mu_gc[0])
            max_energy_fo_error = np.append(
                max_energy_fo_error, max(energy_fo_error[3::])
            )
    ...
    def test_angularmomentum_conservation(self):
        ...
        ma.set_dofs([0, 0, R0test + 0.01, 0, R0test + 0.01, 0.0, 0, 0.0, 0.0])
        ...
        for i in range(nparticles):
            ...
            for j in range(N):
                v_gc = gc_ty[idxs[j], 4]
                r_squared = (
                    np.sqrt(gc_xyzs[j][0] ** 2 + gc_xyzs[j][1] ** 2) - R0test
                ) ** 2 + gc_xyzs[j][2] ** 2
            ...
            pphi1_error = np.log10(
                np.abs(pphi1_gc - pphi1_gc[0] + 1e-30) / np.abs(pphi1_gc[0])
            )
            pphi2_error = np.log10(
                np.abs(pphi2_gc - pphi2_gc[0] + 1e-30) / np.abs(pphi2_gc[0])
            )
            pphi_error = np.log10(
                np.abs(pphi_gc - pphi_gc[0] + 1e-30) / np.abs(pphi_gc[0])
            )
    ...
    def test_energy_momentum_conservation_boozer(self):
        ...
        for i in range(nparticles):
            ...
            for j in range(N):
                ...
                mu_gc = np.append(
                    mu_gc, Ekin / (m * AbsBs_gc[j]) - 0.5 * v_gc**2 / AbsBs_gc[j]
                )
        ...
        gc_tys, gc_phi_hits_2 = trace_particles_boozer(
            bsh,
            stz_inits,
            vpar_inits,
            tmax=tmax,
    ...
    def test_compute_resonances(self):
        ...
        gc_tys, gc_phi_hits = trace_particles_boozer(
            ...
            stopping_criteria=[
                MinToroidalFluxStoppingCriterion(0.01),
                MaxToroidalFluxStoppingCriterion(0.99),
                ToroidalTransitStoppingCriterion(100.0, True),
            ],
            tol=1e-8,
        )
