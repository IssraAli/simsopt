...
class CoilForcesTest(unittest.TestCase):
    ...
    def test_force_and_torque_objectives_with_different_quadpoints(self):
        ...
        for ForceClass, kwargs in [
            ...
            np.testing.assert_allclose(
                obj_all.dJ(), obj_split.dJ(), rtol=1e-2, atol=1e7
            )
                        ...
                        for p in p_list:
                            for threshold in threshold_list:
                                for reg_name, reg_func in regularization_types:
                                    regularization = reg_func()
                                    for downsample in downsample_list:
                                        base_curves_TF = create_equally_spaced_curves(
                                            ncoils, nfp, stellsym
                                        )
                                        base_currents_TF = [
                                            Current(I) for j in range(ncoils)
                                        ]
                                        ...
                                        base_curves = create_equally_spaced_curves(
                                            ncoils, nfp, stellsym, R0=0.5, R1=0.1
                                        )
                                        psc_array = PSCArray(
                                            base_curves,
                                            coils_TF,
                                        ...
                                        for J in objectives:
                                            print(
                                                f"ncoils={ncoils}, nfp={nfp}, stellsym={stellsym}, p={p}, threshold={threshold}, reg={reg_name}, downsample={downsample}, objective={type(J).__name__}"
                                            )
                                            ...
                                            for i in range(11, 18):
                                                ...
                                                if len(errors) > 1 and err_new > 1e-10:
                                                    ...
                                                    print(
                                                        f"err: {err_new}, eps: {eps}, ratio: {ratio}"
                                                    )
                                            ...
                                            if len(errors) > 2:
                                                ...
                                                if len(ratios) > 0:
                                                    ...
                                                    assert median_ratio < threshold, (
                                                        f"Median convergence ratio {median_ratio:.4f} too large (threshold={threshold}). Individual ratios: {ratios}"
                                                    )
                    ...
                    for stellsym in stellsym_list:
                        for reg_name, reg_func in regularization_types:
                            ...
                            for i in range(ncoils):
                                base_currents_TF[i].fix_all()
                            coils_TF = coils_via_symmetries(
                                base_curves_TF, base_currents_TF, nfp, stellsym
                            )
                            base_curves = create_equally_spaced_curves(
                                ncoils, nfp, stellsym, R0=0.5, R1=0.1
                            )
                            ...
                            for J in objectives:
                                print(
                                    f"ncoils={ncoils}, nfp={nfp}, stellsym={stellsym}, objective={type(J).__name__}"
                                )
                                J.x = dofs
                                psc_array.recompute_currents()
                                dJ = J.dJ()
                                ...
                                for i in range(11, 18):
                                    ...
                                    if np.abs(deriv) < 1e-8:
                                        ...
                                        err_new = np.abs(deriv_est - deriv) / np.abs(
                                            deriv
                                        )
                                    errors.append(err_new)
                                    epsilons.append(eps)
                                ...
                                if len(errors) > 2:
                                    ratios = [
                                        (errors[i] + 1e-12) / (errors[i - 1] + 1e-12)
                                        for i in range(1, len(errors))
                                        if errors[i - 1] > 1e-10
                                    ]
                                    if len(ratios) > 0:
