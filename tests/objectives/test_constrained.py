...
class ConstrainedProblemTests(unittest.TestCase):
    ...
    def test_fails(self):
        ...
        np.testing.assert_allclose(
            prob.nonlinear_constraints(prob.x + 1), np.full(2, prob.fail), 1e-12
        )
    ...
    def test_nonlinear(self):
        ...
        np.testing.assert_allclose(prob.lhs_nlc, [lb], 1e-12)
        np.testing.assert_allclose(prob.rhs_nlc, [ub], 1e-12)
        correct_val = np.concatenate(
            [lb - (aff.A @ aff.x + aff.B), aff.A @ aff.x + aff.B - ub]
        )
        np.testing.assert_allclose(prob.nonlinear_constraints(), correct_val, 1e-12)
        self.assertTrue(prob.has_nlc == True)
        ...
        prob = ConstrainedProblem(
            rosen.f, tuples_nlc=[(aff.f, lb, ub), (rosen.f, 0.0, 8.0)]
        )
        ...
        correct_val = np.concatenate(
            [
                lb - (aff.A @ aff.x + aff.B),
                aff.A @ aff.x + aff.B - ub,
                [0.0 - rosen.f()],
                [rosen.f() - 8.0],
            ]
        )
