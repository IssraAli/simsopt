...
class TestContinuation(unittest.TestCase):
    def test_continuation_basic(self):
        ...
        with ScratchDir("."):
            ...
            biot_savart_files = list(
                Path("qa_output_continuation/").glob("*/biot_savart.json")
            )
            self.assertGreater(len(biot_savart_files), 0)
...
class TestRealOptimizationRun(unittest.TestCase):
    def test_initial_optimizations_and_continuation(self):
        ...
        with ScratchDir("."):
            ...
            make_stage_II_pareto_plots(
                df, df_filtered, OUTPUT_DIR="qa_output_continuation/"
            )
            ...
            continuation_results = list(
                Path("qa_output_continuation/").glob("*/results.json")
            )
            continuation_hist = list(
                Path("qa_output_continuation/").glob("histograms.pdf")
            )
    ...
    def test_make_stage_II_pareto_plots_with_df_filtered(self):
        ...
        with ScratchDir("."):
            ...
            self.assertGreater(
                histograms[0].stat().st_size, 0, "histograms.pdf should be non-empty"
            )
...
class TestOptimizationKwargs(unittest.TestCase):
    ...
    def test_continuation_with_all_kwargs(self):
        ...
        with ScratchDir("."):
            ...
            # Check that a results.json file was created
            results_files = list(
                Path("qa_output_kwargs_continuation/").glob("*/results.json")
            )
            self.assertGreater(len(results_files), 0)
    ...
    def test_coil_optimization_basic(self):
        ...
        with ScratchDir("."):
            ...
            coils = coils_via_symmetries(
                base_curves, base_currents, nfp, True, regularizations
            )
            ...
            final_BdotN = np.mean(
                np.abs(
                    np.sum(
                        bs_optimized.B().reshape((nphi, ntheta, 3)) * s.unitnormal(),
                        axis=2,
                    )
                )
            )
            ...
            np.testing.assert_allclose(
                bs_optimized.get_points_cart_ref(), points, atol=1e-8
            )
