    ...
    def test_generate_curves_and_remove_inboard(self):
        ...
        with ScratchDir("."):
            ...
            dists_after = np.array(
                [np.min(np.linalg.norm(curve.gamma(), axis=1)) for curve in filtered]
            )
            print(dists_after)
            ...
            filtered2 = remove_interlinking_dipoles_and_TFs(
                base_wp_curves, base_tf_curves, eps=0.05
            )
    ...
    def test_initialize_coils(self):
        ...
        with ScratchDir("."):
            for config, surf_file in config_file_map.items():
                surf_path = (
                    Path(__file__).parent / ".." / "test_files" / surf_file
                ).resolve()
                try:
                    s = SurfaceRZFourier.from_vmec_input(
                        surf_path, range="half period", nphi=nphi, ntheta=ntheta
                    )
                except:
                    s = SurfaceRZFourier.from_wout(
                        surf_path, range="half period", nphi=nphi, ntheta=ntheta
                    )
                base_curves, curves, coils, base_currents = initialize_coils(
                    s, config, regularization
                )
                ...
                print(
                    f"B at major radius for {config} is {B_magnitude}, should be 5-6 T"
                )
                self.assertTrue(
                    np.isclose(B_magnitude, 5.7, atol=1),
                    f"B at major radius for {config} is {B_magnitude}, expected ~5.7 T",
                )
    ...
    def test_generate_windowpane_array_and_tf_array(self):
        ...
        with ScratchDir("."):
            ...
            for curve in tf_curves_planar:
                ...
                self.assertTrue(
                    np.allclose(dots, 0, atol=1e-12), "TF coil is not planar"
                )
                # Check for free dofs R0 and r_rotation
                dof_names = getattr(curve, "local_dof_names", None)
                self.assertIsNotNone(
                    dof_names, "Curve does not have free_names attribute"
                )
            ...
            self.assertIn(
                "r_rotation",
                dof_names_unfixed,
                "Curve does not have free dof r_rotation",
            )
            self.assertIn(
                "phi_rotation",
                dof_names_unfixed,
                "Curve does not have free dof phi_rotation",
            )
            self.assertIn(
                "z_rotation",
                dof_names_unfixed,
                "Curve does not have free dof z_rotation",
            )
            # --- End new test ---
            ...
            class DummyObj(Optimizable):
                ...
                def dJ(self):
                    return np.ones(len(wp_curves[0].x))
