...
class PermanentMagnetGridTesting(unittest.TestCase):
    def test_bad_params(self):
        ...
        with ScratchDir("."):
            ...
            with self.assertRaises(ValueError):
                PermanentMagnetGrid.geo_setup_between_toroidal_surfaces(
                    s, np.zeros((nphi, ntheta // 2)), s1, s2
                )
            with self.assertRaises(ValueError):
            ...
            with self.assertRaises(ValueError):
                PermanentMagnetGrid.geo_setup_from_famus(
                    s, np.zeros((nphi, ntheta)), TEST_DIR / "zot80.log"
                )
            with self.assertRaises(ValueError):
                kwargs = {"m_maxima": np.ones(4)}
                PermanentMagnetGrid.geo_setup_from_famus(
    ...
    def test_optimize_bad_parameters(self):
        ...
        with ScratchDir("."):
            ...
            base_curves = create_equally_spaced_curves(
                2, s.nfp, stellsym=False, R0=1.0, R1=0.5, order=5
            )
            ...
            with self.assertRaises(ValueError):
                (
                    ...
                    _,
                    _,
                ) = relax_and_split(pm_opt, m0=np.zeros(pm_opt.ndipoles * 3 // 2))
            ...
            with self.assertRaises(ValueError):
                (
                    ...
                    _,
                    _,
                ) = relax_and_split(pm_opt, m0=np.zeros((pm_opt.ndipoles, 3)))

            with self.assertRaises(ValueError):
                mmax = np.ravel(
                    np.array([pm_opt.m_maxima, pm_opt.m_maxima, pm_opt.m_maxima]).T
                )
                (
                    _,
                    _,
                    _,
                ) = relax_and_split(pm_opt, m0=np.ones((pm_opt.ndipoles * 3)) * mmax)
    ...
    def test_projected_normal(self):
        ...
        with ScratchDir("."):
            ...
            p = SurfaceRZFourier.from_vmec_input(
                filename, range="half period", nphi=nphi, ntheta=ntheta
            )
    ...
    def test_Bn(self):
        ...
        with ScratchDir("."):
            ...
            # check <Bn>
            B_opt = np.mean(
                np.abs(pm_opt.A_obj.dot(pm_opt.m) - pm_opt.b_obj)
            ...
            f_B_Am = (
                0.5
                * np.linalg.norm(pm_opt.A_obj.dot(pm_opt.m) - pm_opt.b_obj, ord=2) ** 2
            )
        ...
        # check <Bn>
        B_opt = np.mean(
            np.abs(pm_opt.A_obj.dot(pm_opt.m) - pm_opt.b_obj) * np.sqrt(Ngrid / Nnorms)
        )
        B_dipole_field = np.mean(
            np.abs(
                np.sum(
        ...
        # check integral Bn^2
        f_B_Am = (
            0.5 * np.linalg.norm(pm_opt.A_obj.dot(pm_opt.m) - pm_opt.b_obj, ord=2) ** 2
        )
        f_B = SquaredFlux(s, b_dipole, -Bn).J()
        np.testing.assert_allclose(f_B, f_B_Am, atol=1e-15)
    ...
    def test_grid_chopping(self):
        ...
        for range_type in ["full torus", "half period", "field period"]:
            s = SurfaceRZFourier.from_vmec_input(
                filename, range=range_type, nphi=nphi, ntheta=ntheta
            )
            s1 = SurfaceRZFourier.from_vmec_input(
                filename, range=range_type, nphi=nphi, ntheta=ntheta
            )
            ...
            match_tol = 0.1
            r_cartesian = np.sqrt(
                pm_opt.dipole_grid_xyz[:, 0] ** 2 + pm_opt.dipole_grid_xyz[:, 1] ** 2
            )
            ...
            s1 = SurfaceRZFourier.from_vmec_input(
                filename, range=range_type, nphi=nphi, ntheta=2 * ntheta
            )
            s2 = SurfaceRZFourier.from_vmec_input(
                filename, range=range_type, nphi=nphi, ntheta=2 * ntheta
            )
            ...
            s1 = SurfaceRZFourier.from_vmec_input(
                filename, range=range_type, nphi=2 * nphi, ntheta=ntheta
            )
            s2 = SurfaceRZFourier.from_vmec_input(
                filename, range=range_type, nphi=2 * nphi, ntheta=ntheta
            )
    ...
    def test_famus_functionality(self):
        ...
        with ScratchDir("."):
            ...
            for i in range(mag_data.nMagnets):
                assert np.isclose(
                    np.dot(
                        np.array(mag_data.perp_vector([i])).T,
            ...
            assert np.allclose(
                mag_data.oz[: mag_data.nMagnets // 2],
                -mag_data.oz[mag_data.nMagnets // 2 :],
            )
            with self.assertRaises(ValueError):
            ...
            phi0 = np.pi / 2
            ox2, oy2, oz2 = stell_point_transform(
                "reflect", phi0, mag_data.ox, mag_data.oy, mag_data.oz
            )
            assert np.allclose(mag_data.oz, -oz2)
            ox2, oy2, oz2 = stell_point_transform(
                "translate", phi0, mag_data.ox, mag_data.oy, mag_data.oz
            )
            assert np.allclose(mag_data.oz, oz2)
            ...
            with self.assertRaises(ValueError):
                nx2, ny2, nz2 = stell_point_transform(
                    "random", phi0, mag_data.ox, mag_data.oy, mag_data.oz
                )
    ...
    def test_polarizations(self):
        ...
        assert np.allclose(
            np.concatenate(
                (pol_f, faceedge_vectors(theta), facecorner_vectors(theta)), axis=0
            ),
            face_triplet(theta, theta),
        )
        assert np.allclose(
            np.concatenate(
                (pol_e, faceedge_vectors(theta), facecorner_vectors(theta)), axis=0
            ),
        ...
        assert np.allclose(
            pol_types, np.ravel(np.array([np.ones(vec_shape), 2 * np.ones(vec_shape)]))
        )
    ...
    def test_pm_helpers(self):
        ...
        with ScratchDir("."):
            ...
            base_curves, base_currents, ncoils = read_focus_coils(
                TEST_DIR / "muse_tf_coils.focus"
            )
            coils = []
            ...
            s = SurfaceRZFourier.from_wout(
                TEST_DIR / "wout_LandremanPaul2021_QA_lowres.nc",
                range="half period",
                nphi=nphi,
                ntheta=ntheta,
            )
        ...
        with ScratchDir("."):
            ...
            reg_l0, reg_l1, reg_l2, nu = pm_opt.rescale_for_opt(
                reg_l0, reg_l1, reg_l2, nu
            )
        ...
        with ScratchDir("."):
            ...
            pm_opt = PermanentMagnetGrid.geo_setup_from_famus(
                s, Bn, TEST_DIR / "zot80.focus", **kwargs_geo
            )
            R2_history, Bn_history, m_history = GPMO(pm_opt, "baseline", **kwargs)
            make_optimization_plots(R2_history, m_history, m_history, pm_opt)
            kwargs["K"] = 5
    ...
    def test_pm_post_processing(self):
        ...
        with ScratchDir("."):
            run_Poincare_plots_with_permanent_magnets(
                s_plot, bs, b_dipole, None, "poincare_test"
            )
