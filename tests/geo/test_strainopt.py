...
class CoilStrainTesting(unittest.TestCase):
    def test_strain_opt(self):
        ...
        for centroid in [True, False]:
            ...
            Jb = LPBinormalCurvatureStrainPenalty(
                framedcurve, width=1e-3, p=2, threshold=0
            )
            J = Jt + Jb
            ...
            minimize(
                ...
                jac=True,
                method="L-BFGS-B",
                options={"maxiter": 100, "maxcor": 10, "gtol": 1e-20, "ftol": 1e-20},
                tol=1e-20,
            )
            assert Jt.J() < 1e-12
            assert Jb.J() < 1e-12
    ...
    def subtest_binormal_curvature(self, order, centroid):
        ...
        J = LPBinormalCurvatureStrainPenalty(
            framedcurve, width=1e-3, p=2, threshold=1e-4
        )
