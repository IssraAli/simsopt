#!/usr/bin/env python
r"""
Coupled optimization of TF coil geometry with passive superconducting bulk
pucks (ideal diamagnetic limit).
...
Quaternion DOFs are **unfixed** below so puck orientation is optimized (full JAX
path through :math:`L` assembly per gradient; slower than TF-only). Comment out
the ``unfix`` block to fix puck geometry and use the fast TF-only VJP per step.
...
OUT_DIR = os.path.join(
    ...
    "passive_bulks_optimization_out",
)
...
# ---------------------------------------------------------------------------
# 3. Passive bulk pucks -- large, outside plasma, non-overlapping; quaternions
#    unfixed below so the optimizer can rotate each puck.
# ---------------------------------------------------------------------------
g_plasma = s.gamma()
r_plasma = np.sqrt(g_plasma[:, :, 0] ** 2 + g_plasma[:, :, 1] ** 2)
R_outer = float(r_plasma.max())
puck_offset = 2.5
puck_R = 0.9
puck_t = 0.2
...
n_pucks_phi = 3
z_offsets = [1.1, -1.1]
phi_base = np.linspace(np.pi / (8 * nfp), np.pi / nfp - np.pi / (8 * nfp), n_pucks_phi)
centers_list = []
axes_list = []
for phi in phi_base:
    for z_off in z_offsets:
        r = R_outer + puck_offset
        center = np.array([r * np.cos(phi), r * np.sin(phi), z_off])
...
def _assert_pucks_nonintersecting(psc):
    """Conservative check: sphere around each center with radius R + t/2 must not overlap."""
...
# Unfix quaternion DOFs to optimize puck orientation (slow: full JAX L + VJP).
for i in range(n_base_pucks):
    psc_bulk.unfix(f"q0_{i}")
    psc_bulk.unfix(f"qi_{i}")
    psc_bulk.unfix(f"qj_{i}")
    psc_bulk.unfix(f"qk_{i}")
...
# Call after any change to TF coil DOFs / puck DOFs to ensure
# downstream B evaluations see refreshed modal coefficients.
psc_bulk.recompute_currents()
...
def emit_vtk_snapshot(iteration, include_pucks=True):
    ...
    Bn = np.sum(B * s_plot.unitnormal(), axis=2)
    modB = np.linalg.norm(B, axis=-1)
    pointData = {
        "B_N": Bn[:, :, None],
        "B_N / B": (Bn / (modB + 1e-30))[:, :, None],
    }
