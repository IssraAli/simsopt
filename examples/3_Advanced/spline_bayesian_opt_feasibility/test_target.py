...
INFEASIBLE_RESULT = np.array([0.0, 0.0])  # [dummy_objective, infeasible]

# Lazy-initialized MpiPartition (created on first use)
_mpi = None
...
def parallel_batch_target(candidates, spline_kwargs, lb, ub, stopp):
    ...
    if stopp[0] == 0:
        ...
        if rank == 0:
            ...
            Y_cand = np.array(gathered)  # shape (batch, 2)
            return torch.tensor(Y_cand, dtype=torch.double).reshape(-1, 2)
...
def target(X, spline_kwargs, lb, ub):
    ...
    try:
        ...
        vmec = Vmec.vmec_from_surf(
            nfp=rz_surf.nfp, surf=rz_surf, mpi=get_mpi(), ns=13, M=12, N=12, ftol=1e-7
        )
        # rz_surf.plot()
        vmec.run()
        ...
        qs_penalty = alan_QuasisymmetryRatioResidual(
            vmec, np.linspace(0.02, 1, 20), 1, 0
        )

        qs_s1 = np.sqrt(5) * alan_QuasisymmetryRatioResidual(
            vmec, np.arange(1.0, 1.1, 0.1), 1, 0
        )
        ...
        if np.isnan(res) or np.isinf(res):
            print(f"NaN/Inf objective, marking infeasible")
            return INFEASIBLE_RESULT
        return np.array([float(res), 1.0])  # [objective, feasible]
...
def alan_QuasisymmetryRatioResidual(
    vmec: Vmec,
    ...
    for js in range(ns):
        residuals3d[js, :, :] = (
            np.sqrt(weights[js] * nfp * dtheta * dphi / V_prime[js] * sqrtg[js, :, :])
            * (
