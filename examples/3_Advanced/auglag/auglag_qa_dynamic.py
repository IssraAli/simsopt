#!/usr/bin/env python
"""
auglag_qa_dynamic.py
===============

This script performs the dynamic resolution coil optimizations for the bar plots of Figure 1 of the paper.
...
OUT_DIR = (f"./output_paper/qa__dynamic_ncoils{ncoils}_curvature{CURVATURE_THRESHOLD}_msc{MSC_THRESHOLD}_" + \
...
for min_coil_mode in range(order,order+1):
    ...
    # for c in base_curves:                                                                                                       
    #     print(c.dofs_free_status)

    x, fnc, lag_mul = augmented_lagrangian_method(f=f,
        equality_constraints=c_list,
        tau=4, #4 for 14, 5 for 12, 5 for 11, 5 for 10, 6 for 9.5, 5 for 9
        MAXITER=MAXITER, 
        MAXITER_lag=50, #MAXITER_lag,
        grad_tol=1e-8,
        c_tol=1e-8,
    )
...
np.savez(OUT_DIR + 'opt_data.npz', BdotN = avg_BdotN_over_Bs, forces = max_forces, lengths = lengths, 
